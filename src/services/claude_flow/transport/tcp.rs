...
use crate::services::claude_flow::types::{JsonRpcRequest, JsonRpcResponse};
...
impl Transport for TcpTransport {
    ...
    async fn disconnect(&mut self) -> Result<()> {
        ...
        if let Some(stream) = self.stream.take() {
            if let Ok(mut s) = stream.lock().await.try_into_std() {
    ...
    async fn request(&mut self, request: JsonRpcRequest) -> Result<JsonRpcResponse> {
        ...
        match timeout(Duration::from_secs(30), self.receive()).await {
        ...
        if let Some(stream) = self.stream.take() {
            if let Ok(stream) = Arc::try_unwrap(stream) {
                if let Ok(stream) = stream.into_inner() {
                    if let Ok(mut s) = stream.try_into_std() {
                        let _ = s.shutdown(std::net::Shutdown::Both);
                    }
                }
            }
...
mod tests {
    ...
    async fn test_tcp_transport_creation() {
        let transport = TcpTransport::new("localhost", 9500);
        assert_eq!(transport.host, "localhost");
        assert_eq!(transport.port, 9500);
        assert!(!transport.is_connected());
    }

    #[tokio::test]
    async fn test_tcp_transport_env_defaults() {
        std::env::set_var("CLAUDE_FLOW_HOST", "test-host");
        std::env::set_var("MCP_TCP_PORT", "9600");
        
        let transport = TcpTransport::new_with_defaults();
        assert_eq!(transport.host, "test-host");
        assert_eq!(transport.port, 9600);
        
        std::env::remove_var("CLAUDE_FLOW_HOST");
        std::env::remove_var("MCP_TCP_PORT");
    }
}