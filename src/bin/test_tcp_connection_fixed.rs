...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("File descriptors after connect: {} (delta: +{})", 
          post_connect_fd_count, post_connect_fd_count - initial_fd_count);
    ...
    info!("Final file descriptors: {} (delta: {:+})", final_fd_count, fd_delta);
    
    if fd_delta > 0 {
        error!("⚠️  File descriptor leak detected! {} descriptors not cleaned up", fd_delta);
    } else {
    ...
    println!("Resource leak status: {}", if fd_delta > 0 { "LEAK DETECTED" } else { "CLEAN" });
