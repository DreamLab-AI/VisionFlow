...
// Note: This migration script is obsolete - settings are now stored exclusively in Neo4j.
// sqlite_settings_repository was removed during SQLite → Neo4j migration.
...
async fn migrate_settings(config: MigrationConfig) -> Result<MigrationStats> {
    ...
    // MIGRATION SCRIPT OBSOLETE: SqliteSettingsRepository was removed during Neo4j migration.
    // Settings are now stored exclusively in Neo4j.
    anyhow::bail!("This migration script is obsolete. Settings are now stored exclusively in Neo4j.");
    ...
    // Initialize Neo4j repository (DELETED)
    ...
    //
    // // Migrate individual settings (DELETED)
    ...
    //             } else {
    //                 match neo4j_repo.set_setting(key, value, Some("Migrated from SQLite")).await {
    //                     Ok(_) => {
    //                         debug!("Migrated: {}", key);
    //                         stats.migrated_settings += 1;
    //                     }
    ...
    // // Migrate physics profiles (DELETED)
    ...
    // }
    //
    // // Migrate full settings snapshot (DELETED)
