    ...
    fn apply_physicality_cluster_forces_cpu(&self, graph: &mut GraphData) {
        for (i, node) in graph.nodes.iter_mut().enumerate() {
            ...
            for j in 0..graph.nodes.len() {
                ...
                let dx = node.data.x - graph.nodes[j].data.x;
                let dy = node.data.y - graph.nodes[j].data.y;
                let dz = node.data.z - graph.nodes[j].data.z;
                let dist = (dx * dx + dy * dy + dz * dz).sqrt();
                ...
                if dist < self.config.physicality_cluster.cluster_radius * 2.0 && dist > 0.001 {
                    ...
                    node.data.vx += dx * force / dist;
                    node.data.vy += dy * force / dist;
                    node.data.vz += dz * force / dist;
                }
            }
        }
    }
