use actix_web::{HttpRequest, HttpResponse, Result, dev::ServiceRequest};
...
impl RateLimiter {
    ...
    pub fn reset_time(&self, client_id: &str) -> Duration {
        ...
        if let Some(entry) = clients.get(client_id) {
            ...
            let time_to_next_token = Duration::from_secs(60 / self.config.requests_per_minute as u64);
    ...
    fn cleanup_if_needed(&self) {
        ...
        *last_cleanup = now;
        ...
        clients.retain(|_, entry| {
            ...
            !entry.is_banned() || 
            entry.banned_until.map(|until| now < until + Duration::from_secs(3600)).unwrap_or(true)
        });

        let after_count = clients.len();
        if before_count != after_count {
            debug!("Rate limiter cleanup: removed {} expired entries", before_count - after_count);
        }
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                clients_guard.retain(|_, entry| {
                    ...
                    let keep_banned = entry.banned_until
                        .map(|until| now < until + Duration::from_secs(3600))
                        .unwrap_or(false);
                    
                    let keep_active = now.duration_since(entry.last_refill) < Duration::from_secs(1800); // 30 minutes
                ...
                if before_count != after_count {
                    info!("Rate limiter background cleanup: removed {} expired entries", 
    ...
    pub fn get_stats(&self) -> RateLimitStats {
        ...
        let banned_clients = clients.values()
...
pub fn create_rate_limit_response(client_id: &str, limiter: &RateLimiter) -> Result<HttpResponse> {
    ...
    let response = if limiter.is_banned(client_id) {
