...
mod tests {
    ...
    fn create_ai_metadata() -> MetadataStore {
        ...
        let ai_topics = [("artificial_intelligence", 20), ("technology", 10)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        let ml_topics = [("machine_learning", 25), ("artificial_intelligence", 15)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        let dl_topics = [("deep_learning", 30), ("machine_learning", 20)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        let nn_topics = [("neural_networks", 35), ("deep_learning", 25)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        store.insert("ai_overview.md".to_string(), Metadata {
    ...
    fn create_mixed_metadata() -> MetadataStore {
        ...
        let cooking_topics = [("cooking", 40), ("recipes", 30)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        let travel_topics = [("travel", 35), ("tourism", 25)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
    ...
    fn create_hierarchical_metadata() -> MetadataStore {
        ...
        let overview_topics = [("overview", 30), ("index", 20)].into_iter()
            .map(|(k, v)| (k.to_string(), v)).collect();
        
        store.insert("overview.md".to_string(), Metadata {
        ...
        store.insert("chapter_1.md".to_string(), Metadata {
            file_size: 8000,
            hyperlink_count: 3,
            ..Default::default()
        });
        
        store.insert("chapter_2.md".to_string(), Metadata {
            file_size: 7000,
            hyperlink_count: 2,
            ..Default::default()
        });
        
        store.insert("section_1.1.md".to_string(), Metadata {
            file_size: 3000,
            hyperlink_count: 1,
            ..Default::default()
        });
    ...
    fn create_test_constraints() -> ConstraintSet {
        ...
        constraint_set.add(crate::models::constraints::Constraint::fixed_position(1, 0.0, 0.0, 0.0));
        constraint_set.add(crate::models::constraints::Constraint::separation(2, 3, 100.0));
