...
impl GraphExportHandler {
    ...
    async fn check_rate_limit(&self, client_ip: &str) -> Result<RateLimitInfo> {
        ...
        let state = rate_limits.entry(client_ip.to_string())
        ...
        let hourly_count = state.requests.iter()
    ...
    async fn get_current_graph(&self, _app_state: &AppState) -> Result<GraphData> {
        ...
        for i in 1..=9 {
            graph.edges.push(crate::models::edge::Edge::new(i, i + 1, 1.0));
        }
...
pub async fn export_graph(
    ...
    let client_ip = req.connection_info().remote_addr()
    ...
    // Export graph
    match handler.serialization_service.export_graph(&graph, &request).await {
        Ok(export_response) => {
            Ok(HttpResponse::Ok().json(export_response))
        }
        Err(e) => {
...
pub async fn share_graph(
    ...
    // Create shared graph
    match handler.serialization_service.create_shared_graph(&graph, &request).await {
