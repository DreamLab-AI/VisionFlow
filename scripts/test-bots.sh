...
# Test script for bots visualization

echo "🐝 Testing Bots Visualization Integration..."

# Check if services are running
echo "Checking services..."
supervisorctl status

# Send test bots data to the backend
echo -e "\n📊 Sending test bots data to backend..."
curl -X POST http://localhost:3001/api/bots/update \
...
# Get bots data from backend
echo -e "\n📥 Fetching bots data from backend..."
curl -X GET http://localhost:3001/api/bots/data | jq '.'

echo -e "\n✅ Test complete! Check the browser at http://192.168.0.51:3001 to see the bots visualization."
echo "The bots should appear as a separate graph with gold/green nodes showing agent activity."