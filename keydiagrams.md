...
- {{embed ((69a58988-18bc-47f4-8316-5feca57907a7))}}
- ## Expectations
	- This isn't a product pitch, the software is open source, under continuous agentiic engineering development.
- ## VisionFlow / IRIS — Spatial Intelligence for the Age of Agents
	- ![Slide1.png](../assets/Slide1_1770741359300_0.png)
- # Technology Stack
	- I'll talk about both today - switching back and forth and responding to real time questions.
		- ## Engine (dockerised multi agent orchestration)
			- The primary focus is the docker stack supporting agent activity.
			- AI agents interact with the knowledge graph through 7 MCP (Model Context Protocol) ontology tools, forming a closed loop: discover → read → propose → consistency check → GitHub PR → human review → merge → re-reason → re-layout.
			- The system includes 101 agent skills spanning reasoning, development, orchestration, creative production, and infrastructure management.
			- All knowledge changes are Git commits, providing a complete audit trail.
			- Crucially it's connected heavily to GenAI media tools and can develop workflows.
			- ### Multi-Agent Container Architecture
			  collapsed:: true
				- The Turbo Flow container hosts four AI user personas under `supervisord`, coordinated by `ruv-flow` (formerly `claude-flow`) over shared PostgreSQL vector memory.
				- ```mermaid
				  graph TD
				    subgraph Container["Turbo Flow Unified Container"]
				      SUP["supervisord"]
				      D["devuser · Claude<br/>UID 1000"]
				    ...
				    subgraph Ext["External Services"]
				      RV["RuVector PG<br/>pgvector + HNSW<br/>1.17M+ entries"]
				      NEO["Neo4j 5.13"]
				      GH["GitHub"]
				    end
				    SUP --> D
				    ...
				    RF -->|PR loop| GH
				  ```
				- | User | UID | AI Backend | Purpose |
				  | ---- | ---- | ---- | ---- |
				  | `devuser` | 1000 | Claude Code | Primary orchestration, coding, agent coordination |
...
- # The Problem: AI Is Making Knowledge Work Harder
	...
	- ## The problem is not the AI, but perhaps the interface
...
- ## Why this matters to me
	...
	- It's unbounded by frameworks which age very quickly in this moment. I get value from the development process, the tooling the system relies on for its flexibility, and the knowledge management.
	- This informs my teaching, consulting, training.
	- Other people decide what to do with the cheat codes I have assembled.
	- This isn't currently a polished product, I've been too busy building it to worry about applications, but it's incredibly promising.
	- Productising isn't my critical path but it's an option.
- ## Why this matters for THG
	...
	- ### Opportunity
		- A spatial, ontology-grounded workspace could let category managers, data scientists, and supply chain analysts *see* relationships across the entire portfolio instead of querying them one at a time, and can **underpin trusted and secure media asset creation at velocity and scale**.
- # History and Architecture
	...
	- A TODO list is of course the **CLASSIC** first thing people build.
	...
	- ## 18 months later (full stack spatial intelligence platform)
...
- # What is VisionFlow / IRIS?
	...
	- ## Co-Creation: Specialists + Agents in Private, Grounded Data
		...
		- ```mermaid
		  graph LR
		    subgraph Humans["Human Specialists"]
		      DS["Data Scientist"]
		      DE["Domain Expert"]
		      PM["Project Manager"]
		    end
		    ...
		    subgraph Data["Private Grounded Data"]
		      OWL["OWL Ontology"]
		      KG["Knowledge Graph"]
		      POD["Solid Pod<br/>(private)"]
		    end
		    DS --> KG
		    DE --> OWL
		    PM --> KG
		    ...
		    KG --> POD
		  ```
		- **Grounded**: All work is anchored in formal OWL ontology — agents cannot hallucinate structure because Whelk-rs consistency checking gates every change.
		- **Private**: Solid pods provide per-user data ownership. Ontology namespaces control which agents can read/write which domains. Access is enforced at the MCP tool boundary.
		- **Visible**: Every participant — human or AI — appears in the 3D graph. Agents render as **bioluminescent capsules** (`AgentCapsuleMaterial`) with status-driven colour and workload-driven scale.
	- ## Agents in the Graph: Data Touch Indicators
	  collapsed:: true
		- Agents are first-class entities in the 3D visualisation, not hidden background processes. The binary WebSocket protocol flags agent nodes at the wire level — bit 31 (`0x80000000`) for agents, bit 30 (`0x40000000`) for knowledge nodes — so the client routes them to separate rendering pipelines.
		- ### Agent Status Colours
			- | Status | Colour | Visual |
			  | ---- | ---- | ---- |
			  | Active | Green `#2ECC71` | Working on a task |
			  | Busy | Orange `#F39C12` | Processing, not interruptible |
			  | Idle | Grey `#95A5A6` | Awaiting assignment |
			  | Error | Red `#E74C3C` | Fault state |
			  | Queen | Gold `#FFD700` | Swarm coordinator |
		- ### Data Touch Indicators
			- When an agent (or human) modifies a knowledge node, the node's visual brightness tracks **recency** — recently touched nodes glow brighter, stale ones dim. Computed per-frame from `lastModified` timestamps in the texture buffer alpha channel. This gives every participant a spatial sense of *where work is happening right now*.
			- ```mermaid
			  graph LR
			    A["Agent modifies node"] --> TS["lastModified updated"]
			    TS --> R["computeRecency()"]
			    R --> TB["texBuf alpha channel"]
			    TB --> GPU["Per-frame brightness"]
...
- {{embed ((69a6e650-d962-4a1e-871d-7bc72b4beb1e))}}
	-
...
- # Built on Claude Code / Flow
	...
	- ## The Container: Turbo Flow
	  collapsed:: true
		- The development environment is a single Docker container running four AI users under `supervisord`, connected via shared filesystem, tmux, and PostgreSQL vector memory.
		- ```mermaid
		  graph TB
		    ...
		    MCP --> SK
		  ```
		- Each AI user has its own UID, home directory, and shell profile — they share the filesystem but have isolated process contexts. `ruv-flow` orchestrates them into swarm topologies (hierarchical, mesh, hierarchical-mesh) with configurable max agents and consensus algorithms (raft, byzantine, gossip).
	- ## RuVector: Cross-Agent Vector Memory
	  collapsed:: true
		...
		- ```mermaid
		  graph LR
		    CL["Claude"] -->|store| RV["RuVector PG<br/>1.17M+ entries"]
		    GM["Gemini"] -->|search| RV
		...
		- | Table | Purpose | Scale |
		  | ---- | ---- | ---- |
		  | `memory_entries` | Key-value with vector embeddings | 1.17M+ rows |
		  | `patterns` | Learned development patterns | Cross-session |
		  | `reasoning_patterns` | ReasoningBank trajectories | Per-task |
		  | `sona_trajectories` | Self-optimising learning paths | Persistent |
		  | `session_state` | Agent session continuity | Per-agent |
		- **Why this matters**: When Claude discovers a pattern ("Neo4j adapter swap: 1 file change"), it stores it. When Gemini encounters a similar task, semantic search surfaces that pattern. Knowledge compounds across agents and sessions.
	- ## MCP: The Agent-Knowledge Loop
	  collapsed:: true
		- Model Context Protocol tools are the only interface agents have to the knowledge graph. Seven ontology tools form a closed loop:
		- ```mermaid
		  graph LR
		    D["discover"] --> R["read"]
		    R --> P["propose"]
		    P --> V["validate"]
		    V -->|consistent| PR["GitHub PR"]
		    V -->|inconsistent| P
		    PR --> HU["Human Review"]
		    HU -->|merge| SY["Auto-Sync"]
		    SY --> WH["Whelk Re-Reason"]
		    WH --> GPU["GPU Re-Layout"]
		    GPU --> D
		  ```
		- Additionally, `ruv-flow` exposes MCP memory tools (`memory_store`, `memory_search`, `memory_retrieve`, `memory_list`) and coordination tools (`swarm_init`, `agent_spawn`, `task_orchestrate`) that agents use to self-organise.
		- **The discipline**: Agents cannot bypass the consistency gate. Every proposed change is validated against the Whelk-rs reasoner before it becomes a GitHub PR. Every merged PR triggers re-reasoning and re-layout. The human remains in the loop at the PR review stage.
-
- ---
-
...
- # Examples of Things It's Made
	...
	- Report generated on the day
		- https://github.com/DreamLab-AI/THG-world-record-attempt/blob/main/reports/world-record-report.pdf
...
- # Links & Resources
	...
	- [OG-RAG Paper (EMNLP 2025)](https://aclanthology.org/2025.emnlp-main.1674.pdf) — Ontology performance evidenc
