...
## Related Content: Bitcoin As Money
...
## Related Content: Bitcoin Technical Overview
...
- # Technical Overview
	...
	- ## ECDSA / SHA256 / secp256k1
		...
		- **[Updated 2025]** [[ECDSA]] is being progressively replaced by [[Schnorr signatures]][[schnorr1989efficient]] which use the same [[secp256k1]] curve ensuring [[backward compatibility]]. [[Schnorr signatures]] enable [[signature aggregation]], improved [[privacy]] through [[key aggregation]], and reduced [[transaction]] sizes. As of early 2025, approximately 8-12% of [[Bitcoin]] transactions utilise [[Schnorr signatures]] via [[Taproot]], with adoption accelerating among major [[wallets]] and [[exchanges]].[[Updated 2025]] [[ECDSA]] will remain supported indefinitely for [[backward compatibility]].
	 ...
	 - ### Who is Involved:
				...
				- [[Tim Ruffing]], [[Pedro Moreno-Sanchez]], and [[Yannick Seurin]] are [[cryptographers]] who have published [[peer-reviewed]] [[academic papers]] analysing and improving [[Bitcoin]]'s [[cryptographic]] constructions, particularly around [[signature schemes]] and [[privacy protocols]].
	 ...
	 - [[Time locks]] in [[Bitcoin]] come in two forms: [[block height-based]] (CLTV - CheckLockTimeVerify) and [[time-based]] (CSV - CheckSequenceVerify). [[Miniscript]] enforces clear semantics preventing ambiguous [[time lock]] specifications that could create vulnerabilities. Some [[opcodes]] change behaviour based on parameter encoding length, creating subtle bugs; [[Miniscript]] eliminates these footguns through rigorous [[type checking]].
	 ...
	 - The [[Bitcoin]] [[network]] operates through a triumvirate of [[economic actors]], each with distinct [[incentives]] creating a [[game-theoretic]] equilibrium:
				...
				- **Miners**: Profit from [[coinbase]] rewards and [[transaction fees]], providing computational [[security]] through [[Proof-of-Work]]. [[Miners]] validate other miners' [[blocks]] according to [[consensus rules]] enforced by [[node]] operators, creating a critical [[cheques-and-balances]] system.
				- **Node Operators**: [Enforce consensus rules](https://www.truthcoin.info/blog/measuring-decentralization/) that [[miners]] must satisfy for [[block]] propagation. [[Node]] operators optimise [[trust minimization]] and protect the [[network]] from [[protocol]] changes that might undermine their [[economic]] interests - demonstrated dramatically during the 2017 [[Blocksize Wars]].[[blocksizewars]] Running a [[full node]] provides maximum [[sovereignty]] and [[censorship resistance]].
	 ...
	 - ### HD wallet encoding
	 - ideas
				- The BIP39 standard is designed to create a human-readable and easilyportable format for Bitcoin and other cryptographic wallet seeds. Byrepresenting these seeds as a series of colours in a 3D model, we add anew dimension of portability, visual appeal, and potential applications.
					- **Easter Egg Hunts in Social Metaverses**: The colour-based representation of BIP39 seeds opens up opportunities for creative and engaging experiences in metaverse environments. For instance, mnemonic seeds could be hidden within digital artefacts in the form of colour sequences. These could be used as treasure hunts or easter eggs, potentially carrying real-world value in the form of Bitcoin or other cryptocurrencies. Recovery would mean some kind of sampling as in Fig <a href="#fig:HDwalletBlock" data-reference-type="ref" data-reference="fig:HDwalletBlock">3.12</a> and software.
...
- The GitHub repository for this book [has some examplecode](https://github.com/flossverse/bip39Geom) playing around with thisidea further, generating a colour-based and a three-dimensional graphicalrepresentation of nostr addresses. Each mnemonic word is mapped to aunique colour and a 3D model is created, in which each word of themnemonic is represented by a cube of the corresponding colour arranged ina circle.
-
	 - **BIP39Colors**: A class that contains a list of BIP39 words and methods to convert a hexadecimal seed to RGB colours and mnemonic words.
-
	 - **hex\_to\_rgb**: A static method within the BIP39Colors class that converts a hexadecimal colour to an RGB colour.
-
	 - **seedToColors**: A static method within the BIP39Colors class that converts a seed (a string of hexadecimal characters) into colours and mnemonic words. This function first converts the seed into bytes, then divides these bytes into groups and converts each group into a position in the word list and a colour.
-
	 - **generate\_3d\_model**: A function that takes a list of colours as input, and generates a 3D model with cubes of these colours arranged in a circular formation.
-
	 - **main**: The main function of the script, which takes a 64-character hexadecimal string (a nostr ID) as an argument, converts it to colours and mnemonic words, and generates the 3D model.
...
## Related Content: Bitcoin Value Proposition
...
- ## High level analysis
	- **[Updated 2025]**: [[Bitcoin]]'s [[market capitalization]] reached **$2.22 trillion** as of mid-2025, representing approximately **2.2% of global M2 money supply**. The [[institutional adoption]] narrative has fundamentally shifted with [[BlackRock]]'s [[iShares Bitcoin Trust]] (IBIT) holding over **805,110 BTC** ($100B+ AUM), making it the largest [[ETF]] holder and surpassing - ![image.png](../assets/image_1734450883984_0.png) - - [[MicroStrategy]]'s **640,250 BTC**. Total [[Bitcoin ETF]] assets under management exceeded **$100 billion**, with U.S. [[spot Bitcoin ETFs]] registering nearly **$63 billion in net inflows** since their January 2024 debut.[^1][^2] --
...
## Related Content: Lightning and Similar L2

icon:: Ξ

public:: true

aliases:: [[Lightning Network]], [[Bitcoin Layer 2]], [[LN]], [[Bitcoin L2]]

termID:: BTC-L2-0001

belongsToDomain:: [[BlockchainDomain]], [[Layer 2]], [[Payment Protocols]], [[Scaling Solutions]]

primaryDomain:: Bitcoin Layer 2
...
- # Lightning Network and Bitcoin Layer 2 Solutions
	...
	- ### Bitcoin-AI Integration and L402/X402 Protocols
		...
		- **[[L402 Protocol]]**: Developed by [[Lightning Labs]], the [[L402 Protocol]] (formerly LSAT - Lightning Service Authentication Token) combines [[HTTP 402 Payment Required]] status codes with [[Lightning Network|Lightning]] [[Invoice|invoices]] and [[Macaroon|macaroons]] (cryptographic capability tokens) to create a standardised protocol for paid API access. When a client requests a protected resource, the server responds with an [[HTTP 402]] status and a [[Lightning Network|Lightning]] [[Invoice]]. After payment, the client receives a [[Macaroon]] that grants authenticated access to the API.[[osuntokun2020l402]]
	...
	- ### Privacy-Preserving Layer 2 Extensions
		...
		- #### Fedimint
			...
			- **[[Federation]] Architecture**: Utilises [[Byzantine Fault Tolerance|byzantine fault tolerant]] [[Multi-signature|multi-sig]] wallet technology similar to [[Blockstream]]'s [[Liquid Network]], distributing trust across multiple guardians.
	...
	- ### Liquid Network: Federated Sidechain
		...
		- **[Updated 2025]** [[Liquid Network|Liquid]] has been utilised for innovative financial products, including [[El Salvador]]'s [[Bitcoin Bonds]] in partnership with [[Blockstream]]. The network continues to evolve as a platform for [[Bitcoin]]-native securities and tokenized assets.[[elsalvador2024bonds]]
...
## Related Content: Taproot assets
...
- category:: [[Bitcoin Protocol]], [[Layer 2]], [[Asset Issuance]], [[Lightning Network]]

- relatedConcepts:: [[RGB Protocol]], [[L402 Protocol]], [[X402 Protocol]], [[AI Agent Economies]], [[Taproot Upgrade]]
...
- # David Chaum & The Evolution of Electronic Cash (eCash)
	...
	- ### Fedimint: Federated Privacy Protocol
	  ...
	  - **Trustless Withdrawal**: Users verify mint solvency through public audits and withdraw funds to Bitcoin mainchain at any time, minimising protocol-level trust assumptions.
