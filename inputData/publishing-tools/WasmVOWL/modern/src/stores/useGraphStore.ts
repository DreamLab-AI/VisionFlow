...
interface GraphState {
  ...
  updateGraphData: (nodes: any[], edges?: any[]) => void;
...
export const useGraphStore = create<GraphState>()(
  immer((set, get) => ({
    ...
    loadOntology: (data: OntologyData) => {
      ...
      set((state) => {
        ...
        clonedData.class?.forEach((cls: any) => {
          ...
          // Random initial positions - D3 simulation will handle layout
          const angle = Math.random() * Math.PI * 2;
          const radius = Math.random() * 500;
          const node: Node = {
            id: cls.id,
            type: 'class',
            label: (typeof classAttrs.label === 'string' ? classAttrs.label : classAttrs.label?.en || classAttrs.label?.default) ||
              (typeof cls.label === 'string' ? cls.label : cls.label?.en || cls.label?.default) ||
              cls.id,
            ...
            properties: {
              ...
              term_id: classAttrs.term_id || cls.term_id  // For page navigation
            }
          };
        ...
        clonedData.property?.forEach((prop: any) => {
          ...
          const edge: Edge = {
            ...
            source: source || '',
            target: target || '',
            type: prop.type === 'rdfs:SubClassOf' ? 'subclass' :
              prop.type === 'owl:ObjectProperty' || prop.type === 'owl:objectProperty' ? 'objectProperty' :
                prop.type === 'owl:DatatypeProperty' || prop.type === 'owl:datatypeProperty' ? 'datatypeProperty' :
                  prop.type === 'owl:DisjointWith' ? 'disjoint' : 'objectProperty',
            label: (typeof propAttrs.label === 'string' ? propAttrs.label : propAttrs.label?.en || propAttrs.label?.default) ||
              (typeof prop.label === 'string' ? prop.label : prop.label?.en || prop.label?.default) ||
              prop.id,
            iri: prop.iri || propAttrs.iri,
            properties: {
    ...
    updateGraphData: (data: any[], edges?: any[]) => {
      set((state) => {
        // Clone data to avoid Immer frozen object errors with WASM data
        const clonedData = Array.isArray(data) ? JSON.parse(JSON.stringify(data)) : [];
        ...
        clonedData.forEach((d: any) => {
          ...
          if (node) {
            ...
            if (d.visible !== undefined) {
              ...
              if (d.visible) {
                state.filteredNodes.add(d.id);
              } else {
                state.filteredNodes.delete(d.id);
              }
            }
          }
        });

        // If edges are provided from WASM, use them directly for filtering
        if (edges && Array.isArray(edges)) {
          state.filteredEdges.clear();
          edges.forEach((e: any) => {
            if (state.edges.has(e.id)) {
              // CRITICAL: Ensure endpoints are visible before adding edge
              // This protects against older WASM versions sending all edges
              const edge = state.edges.get(e.id);
              if (edge && state.filteredNodes.has(edge.source) && state.filteredNodes.has(edge.target)) {
                state.filteredEdges.add(e.id);
              }
            }
          });
          console.log(`[useGraphStore] Updated filtered edges: ${state.filteredEdges.size} visible out of ${edges.length} provided`);
        }
      });

      // Apply edge filtering based on node visibility only if edges weren't provided
      if (!edges) {
        get().applyHierarchyEdgeFilter();
      }
    },
    ...
    applyFilters: () => set((state) => {
      ...
      // Start with all nodes and edges, respecting visibility
    ...
    applyHierarchyEdgeFilter: () => set((state) => {
      const { edges, filteredNodes } = state;
