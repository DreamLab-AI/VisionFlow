 ...
 *
 * Usage:
 *   npx claude-flow@v3alpha hooks pre-task --description "ComfyUI workflow"
 *   npx claude-flow@v3alpha hooks post-task --task-id "workflow-123"
 */
...
class HooksManager {
  ...
  async storeMemory(key, value, namespace = 'coordination') {
    try {
      ...
      execSync(
        `npx claude-flow@v3alpha hooks memory-store --key "${key}" --value '${memoryData}' --namespace "${namespace}"`,
        { stdio: 'pipe' }
      );
  ...
  async retrieveMemory(key, namespace = 'coordination') {
    try {
      ...
      const result = execSync(
        `npx claude-flow@v3alpha hooks memory-retrieve --key "${key}" --namespace "${namespace}"`,
  ...
  async notify(message, level = 'info') {
    try {
      const { execSync } = require('child_process');
      execSync(
        `npx claude-flow@v3alpha hooks notify --message "${message}" --level "${level}"`,
        { stdio: 'pipe' }
      );
    } catch (error) {
...
async function preWorkflow(context) {
  ...
  try {
    const { execSync } = require('child_process');
    execSync(
      `npx claude-flow@v3alpha hooks pre-task --description "ComfyUI Workflow ${workflowId}"`,
      { stdio: 'pipe' }
    );
  } catch (error) {
...
async function postWorkflow(context, result) {
  ...
  try {
    const { execSync } = require('child_process');
    execSync(
      `npx claude-flow@v3alpha hooks post-task --task-id "${workflowId}" --success ${result.success}`,
      { stdio: 'pipe' }
    );
  } catch (error) {
...
async function sessionEnd(sessionId, exportMetrics = true) {
  ...
  if (exportMetrics) {
    try {
      const { execSync } = require('child_process');
      execSync(
        `npx claude-flow@v3alpha hooks session-end --session-id "${sessionId}" --export-metrics true`,
        { stdio: 'pipe' }
      );
    } catch (error) {
...
async function sessionRestore(sessionId) {
  ...
  try {
    const { execSync } = require('child_process');
    execSync(
      `npx claude-flow@v3alpha hooks session-restore --session-id "${sessionId}"`,
      { stdio: 'pipe' }
    );
  } catch (error) {
...
async function registerHooks() {
  ...
  try {
    ...
    execSync(
      `npx claude-flow@v3alpha hooks register --service "comfyui" --handlers "preWorkflow,postWorkflow,onProgress,onError"`,
      { stdio: 'pipe' }
    );
