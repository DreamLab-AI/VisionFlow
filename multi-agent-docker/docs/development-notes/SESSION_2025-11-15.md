...
### 4. Perplexity AI Research Skill ⭐
...
- **Updated** docker-compose.unified.yml:
  - Direct RW mount: `${HOME}/.claude` → `/home/devuser/.claude`
...
- **Fixed** all supervisord service definitions:
  - Changed `/usr/bin/node` → `/usr/local/bin/node`
...
- **Running Services**:
  ...
  - ✅ claude-zai (port 9600)
  - ✅ code-server (port 8080)
  - ✅ gemini-flow
  - ✅ SSH (port 2222)
  - ✅ dbus, dbus-user
- **Not Running**:
  - ❌ VNC/XFCE4 (not showing in supervisorctl)
  - ❌ All MCP servers (skills directory mismatch)
  - ❌ tmux workspace (created manually, not persistent)
...
2. **perplexity** - Real-time research with UK focus ⭐ NEW
...
8. Gemini-Shell - gemini-user (UID 1001) ⭐ NEW
9. OpenAI-Shell - openai-user (UID 1002) ⭐ NEW
10. ZAI-Shell - zai-user (UID 1003) ⭐ NEW
...
### Capabilities Added
- ✅ Real-time AI research (Perplexity)
- ✅ UK-centric prompt optimization
- ✅ Extended multi-user tmux workspace
...
## 🐛 Known Issues (Will Be Fixed by Rebuild)
