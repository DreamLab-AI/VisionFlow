...
// ===== PostgreSQL backend (RuVector) — direct delegation =====
let _pgDirect = null;
let _pgDirectChecked = false;
async function getPgDirect() {
    if (_pgDirectChecked) return _pgDirect;
    _pgDirectChecked = true;
    try {
        const mod = await import('./pg-backend.js');
        const r = await mod.testConnection();
        if (r?.connected) { _pgDirect = mod; } else { _pgDirect = null; }
    } catch { _pgDirect = null; }
    return _pgDirect;
}
...
export async function storeEntry(options) {
    // RuVector PG direct delegation
    const pgd = await getPgDirect();
    if (pgd) {
        try {
            const { key, value, namespace = 'default', tags = [], ttl, upsert = false, generateEmbeddingFlag } = options;
            const pgResult = await pgd.storeEntry({ key, value, namespace, tags, ttl, upsert, generateEmbedding: generateEmbeddingFlag !== false });
            return {
                success: true, id: pgResult.id || pgResult.key, key, namespace,
                stored: true, storedAt: new Date().toISOString(),
                hasEmbedding: pgResult.hasEmbedding === true || pgResult.hasEmbedding === 't',
                embeddingDimensions: pgResult.hasEmbedding ? 384 : null,
                backend: 'ruvector-postgres', storeTime: '0ms',
            };
        } catch { /* fall through */ }
    }
    // ADR-053: Try AgentDB v3 bridge first
...
export async function searchEntries(options) {
    ...
    if (pgd) {
        try {
            const startTime = Date.now();
            const { query, namespace, limit = 10, threshold = 0.0 } = options;
            const pgRows = await pgd.searchEntries({ query, namespace: namespace !== 'all' ? namespace : undefined, limit, threshold });
            return {
                success: true,
                results: (pgRows || []).map(r => ({
                    id: r.key, key: r.key,
                    content: typeof r.value === 'string' ? r.value.substring(0, 200) : JSON.stringify(r.value).substring(0, 200),
                    score: parseFloat(r.similarity) || 0,
                    namespace: r.namespace || 'default',
                })),
                searchTime: Date.now() - startTime,
                searchMethod: 'ruvector-postgres-hnsw',
            };
        } catch { /* fall through */ }
    }
    // ADR-053: Try AgentDB v3 bridge first
...
export async function listEntries(options) {
    ...
    if (pgd) {
        try {
            ...
            const pgRows = await pgd.listEntries({ namespace, limit, offset });
            return {
                success: true,
                entries: (pgRows || []).map(r => ({
                    id: r.key, key: r.key, namespace: r.namespace || 'default',
                    size: typeof r.value === 'string' ? r.value.length : JSON.stringify(r.value || '').length,
                    accessCount: 0,
                    createdAt: r.created_at || new Date().toISOString(),
                    updatedAt: r.updated_at || new Date().toISOString(),
                    hasEmbedding: r.hasEmbedding === true || r.hasEmbedding === 't',
                })),
                total: (pgRows || []).length,
...
export async function getEntry(options) {
    ...
    if (pgd) {
        try {
            ...
            const pgEntry = await pgd.getEntry({ key, namespace });
            if (!pgEntry) return { success: true, found: false, backend: 'ruvector-postgres' };
            const content = typeof pgEntry.value === 'string' ? pgEntry.value : JSON.stringify(pgEntry.value);
            let tags = [];
            try { tags = (typeof pgEntry.metadata === 'string' ? JSON.parse(pgEntry.metadata) : pgEntry.metadata)?.tags || []; } catch {}
            return {
                success: true, found: true, cacheHit: false, backend: 'ruvector-postgres',
                entry: {
                    id: pgEntry.key, key: pgEntry.key, namespace: pgEntry.namespace || 'default',
                    content, accessCount: 0,
                    createdAt: pgEntry.created_at || new Date().toISOString(),
                    updatedAt: pgEntry.updated_at || new Date().toISOString(),
                    hasEmbedding: pgEntry.hasEmbedding === true || pgEntry.hasEmbedding === 't',
                    tags,
                },
            };
        } catch { /* fall through */ }
...
export async function deleteEntry(options) {
    ...
    if (pgd) {
        try {
            const { key, namespace = 'default' } = options;
            const pgResult = await pgd.deleteEntry({ key, namespace });
            return { success: true, deleted: pgResult.deleted, key, namespace, remainingEntries: 0 };
        } catch { /* fall through */ }
    }
    // ADR-053: Try AgentDB v3 bridge first
    const bridge = await getBridge();
