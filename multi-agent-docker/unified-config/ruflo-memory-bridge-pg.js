...
// ===== PostgreSQL backend (RuVector) — delegates to pg-backend.js when available =====
let _pgBackend = null;
let _pgBackendChecked = false;
async function getPgBackend() {
    if (_pgBackendChecked) return _pgBackend;
    _pgBackendChecked = true;
    try {
        _pgBackend = await import('./pg-backend.js');
        const result = await _pgBackend.testConnection();
        if (!result?.connected) {
            _pgBackend = null;
        }
    } catch {
...
export async function bridgeStoreEntry(options) {
    // PG backend delegation
    const pg = await getPgBackend();
    if (pg) {
        try {
            const pgResult = await pg.storeEntry({
                key: options.key,
                value: options.value,
                ...
                generateEmbedding: options.generateEmbeddingFlag !== false,
            });
            return {
                success: true,
                id: pgResult.id || pgResult.key,
                embedding: pgResult.hasEmbedding ? { dimensions: 384, model: 'all-MiniLM-L6-v2' } : undefined,
                guarded: true, cached: true, attested: true, backend: 'ruvector-postgres',
            };
...
export async function bridgeSearchEntries(options) {
    ...
    if (pg) {
        try {
            ...
            const pgRows = await pg.searchEntries({
                query: options.query,
                namespace: options.namespace !== 'all' ? options.namespace : undefined,
            ...
            return {
                ...
                results: (pgRows || []).map(r => ({
                    id: r.key,
                    key: r.key,
                    content: typeof r.value === 'string' ? r.value.substring(0, 60) + (r.value.length > 60 ? '...' : '') : JSON.stringify(r.value).substring(0, 60),
                    score: parseFloat(r.similarity) || 0,
                    namespace: r.namespace || 'default',
                    provenance: `pg-knn:${r.similarity}`,
                })),
...
export async function bridgeListEntries(options) {
    ...
    if (pg) {
        try {
            const pgRows = await pg.listEntries({
                namespace: options.namespace,
                limit: options.limit || 20,
                offset: options.offset || 0,
            });
            const entries = (pgRows || []).map(r => ({
                id: r.key,
                key: r.key,
                namespace: r.namespace || 'default',
            ...
            return { success: true, entries, total: entries.length, backend: 'ruvector-postgres' };
        } catch { /* fall through to SQLite */ }
    }
    const registry = await getRegistry(options.dbPath);
    if (!registry)
...
export async function bridgeGetEntry(options) {
    ...
    if (pg) {
        try {
            const pgEntry = await pg.getEntry({
                key: options.key,
                namespace: options.namespace || 'default',
            });
            if (!pgEntry) {
                return { success: true, found: false, backend: 'ruvector-postgres' };
            }
            ...
            return {
                ...
                entry: {
                    id: pgEntry.key,
                    key: pgEntry.key,
                    namespace: pgEntry.namespace || 'default',
                    content,
                    accessCount: 0,
...
export async function bridgeDeleteEntry(options) {
    ...
    if (pg) {
        try {
            const pgResult = await pg.deleteEntry({
                key: options.key,
                namespace: options.namespace || 'default',
            });
            return {
