...
export const memoryTools = [
    {
        ...
        handler: async (input) => {
            ...
            try {
                ...
                const pgBackend = result.backend === 'ruvector-postgres';
                return {
                    success: result.success,
                    key,
                    namespace,
                    stored: result.success || result.stored,
                    storedAt: new Date().toISOString(),
                    hasEmbedding: pgBackend ? (result.hasEmbedding === true) : !!result.embedding,
                    embeddingDimensions: pgBackend ? result.embeddingDimensions : (result.embedding?.dimensions || null),
                    backend: pgBackend ? 'ruvector-postgres' : 'sql.js + HNSW',
            ...
            try {
                ...
                if (result.found && result.entry) {
                    ...
                    return {
                        ...
                        found: true,
                        backend: result.backend || 'sql.js + HNSW',
                    };
                }
                return {
                    key,
        ...
        handler: async (input) => {
            ...
            try {
                ...
                return {
                    ...
                    backend: result.searchMethod?.includes('postgres') ? 'ruvector-postgres' : 'HNSW + sql.js',
                };
            }
            catch (error) {
                return {
    ...
    },
    {
        ...
        handler: async (input) => {
            ...
            try {
                ...
                return {
                    success: result.deleted || result.success,
                    key,
                    namespace,
