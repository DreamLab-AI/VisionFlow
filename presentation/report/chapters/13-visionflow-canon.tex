...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{diag-identity-spine.png}
\caption{The dual-tier identity spine: one secp256k1 keypair serving as relay identity, HTTP-auth identity, WAC principal, provenance author, and DID subject. Rendered from the ecosystem's diagram-as-code corpus.}
...
What remains open is the distributed BrokerActor, which still lives on an unmerged branch, while main shipped a different architecture in its place: an inline decide-and-inbox handler plus an \texttt{ElevationActor} with a real fenced Oxigraph write-back. The distributed actor is now an unmerged alternative, not a blocking gap in the decision loop. Chapter~\ref{ch:role-transformation} carries the front-end story, where a larger gap remains: the broker still has no decision canvas inside VisionClaw's own client.
...
These are named here, at the point where the principles are introduced, so that a reader is not left believing the canon and the code say the same thing. Related overstatements appear in place above: the NIP-42 relay gate is currently a pubkey allowlist, and the flagship elevation loop still opens its human-approval case without invoking the Whelk consistency gate and emits no \texttt{ConceptElevated} closing event. The systematic four-surface accounting of every such gap lives in Chapter~\ref{ch:gap-register}; the component chapters name their headline gaps and point there rather than reproduce it.
