...
no other. The gap-close sprint pushed that identity down to the individual agent: where an earlier tree stamped a placeholder \texttt{did:nostr:local} at
spawn, each agent now mints its own secp256k1 key at spawn through \texttt{agent-identity.js}, holds it in a \texttt{0600} keyfile stable across a profile, and
carries the resulting \texttt{did:nostr} in its spawn payload (\texttt{6189f47d}), so the principal a downstream surface verifies is the agent's own and not the
box's. Beneath the identity layer, the pod substrate described in Chapter~\ref{ch:solid-pod} makes provenance the default: since the
...
\addlinespace
Learn from outcomes & Trajectory producer live; \texttt{feed\_retrieval} and \texttt{feed\_routing} consumers held behind a Wilson-score sample floor until the corpus clears it & Scaffolded \\
\addlinespace
...
The skills also deserve naming rather than only counting, because several are load-bearing for claims made elsewhere in this report. \texttt{hermes-scheduler} is a persistent cron and interval daemon that wakes an agent to run unattended on a schedule; it is the concrete answer to the proactive, always-on agent fleet the compounding-organisation argument (Chapter~\ref{ch:compounding-org}) assumes but never otherwise locates in code. \texttt{ruvnet-brain} exposes a second source-grounded corpus through \texttt{search\_ruvnet}, baked into the sidecar so grounding does not rest on one retrieval stack. \texttt{pdf-signing} emits signed documents; \texttt{ceramic-search} and \texttt{harness-bridge} are web-research and harness-inspection clients; and \texttt{build-with-quality} carries the evidence-driven-development discipline the report argues for. Listed this way, the routing surface reads as a capability substrate, which is what it is, rather than a number three documents disagreed about.
...
\caption{The AgentBox operations surface: four of the manifest-gated services with live status, rooted in the operator's \texttt{did:nostr} identity. Illustration in the house sketch style.}
...
every verification outcome as they happen, which makes them the natural place to record a contextual-transaction-cost figure per workflow. The April~2026 audit
found that seam unwired: the one hook case where a tool outcome could be observed, \texttt{post-bash}, fell through to a bare log line and touched nothing. The
gap-close sprint wired it. A contextual-transaction-cost emitter now runs on the spine and forwards a token-count-and-handoff envelope end to end
(\texttt{9673624}, \texttt{ceb3401b}), so the accounting the plumbing always allowed is finally taken, and failures are tagged to Cemri and colleagues'
fourteen-mode taxonomy at each of the four emit sites through a shared \texttt{failure-taxonomy.js} rather than to free-text error strings (\texttt{9673624})
\parencite{Cemri2025}. Recommendation~3 of Chapter~\ref{ch:ecosystem-roadmap} still owns the reporting of the first production contextual-transaction-cost
figure, now that the figure is being produced.

The same envelope now carries an authority axis. Each recorded action is tagged with an \texttt{authority\_class} (\texttt{9673624}), and a
\texttt{buildAuthorityGate} check sits in front of \texttt{POST /v1/llm/revoke}: an agent's model access blocks the moment a \texttt{kind-31402} escalation is
raised against it and releases only on a verified \texttt{kind-31403} decision (\texttt{ceb3401b}). This is a live runtime lever for the ``control'' property the
sovereignty definition names, binding an agent's right to act to the forum's signed judgment rather than to a config edit. It is honestly tiered
\texttt{standalone}: the gate runs and is tested, but it has not been exercised against a live multi-relay decision flow. Shortly after this edition's
snapshot the same gate was carried onto the broker decide path itself (\texttt{a70dc4fb}), where a zero-tolerance or escalation action blocks until a verified
approving \texttt{kind-31403} releases it, and the broker-bridge that reports a decision back to VisionClaw was corrected to key case closure on the real
\texttt{writeback\_committed} response and forward the actual deciding pubkey rather than a placeholder \texttt{'unknown'} (finding GOV-1 closed).
...
retrieval and routing, \texttt{feed\_retrieval} and \texttt{feed\_routing}, are now held behind an explicit Wilson-score confidence floor rather than an ad-hoc
switch (\texttt{9673624}): they stay gated off until the trajectory corpus clears the lower Wilson bound, which is the honest way to say ``not enough evidence
yet'' rather than learning from noise.
\end{itemize}
