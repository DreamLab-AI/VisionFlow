...
Taylor's insight was that once you had a reliable hierarchy for routing information, you could use that hierarchy to measure, standardise, and optimise the work itself. This produced extraordinary efficiency gains. It also produced the dysfunctions that organisational theorists have spent the subsequent century attempting to remedy. The hierarchy became rigid. Information flowed up but not sideways. Specialisation created silos. The coordination tax (the cost of maintaining all those layers of information routing) grew as organisations grew.
...
The information-routing framing developed in this chapter has a second lineage, one that comes from economics rather than military history. In 1937, the economist Ronald Coase asked a deceptively simple question in his essay ``The Nature of the Firm'': if markets allocate resources so efficiently through the price mechanism, why do firms exist at all? Coase's answer was that markets are not free to use: discovering prices, negotiating contracts, and monitoring compliance all cost something. A firm exists precisely where the cost of coordinating an activity through the market exceeds the cost of coordinating that same activity inside a hierarchy. Decades later, Oliver Williamson extended this insight into a full transaction-cost economics, showing that the choice between market and hierarchy, and every intermediate arrangement between them, is governed by the relative cost of the transactions each requires: search, negotiation, monitoring, and enforcement.

This is the same argument as the one developed above, told in a different vocabulary. Where the Roman legion and the Prussian General Staff solved a bandwidth problem, Coase and Williamson solved a cost problem; the two turn out to be related. Hierarchy is what an organisation buys when the cost of routing information and enforcing agreements through open markets exceeds the cost of routing it through a chain of command. The coordination tax this report has been describing is, in Coase's terms, a transaction cost, paid out in the salaries of people whose sole function is to move information across a boundary.

The agent era demands an extension to this theory, not its abandonment. When the coordinators doing the routing are themselves computational (agents rather than employees), the costly transaction is no longer a human negotiating a contract or monitoring a subordinate. It is the movement of context across a boundary: the prompts, schemas, memory, and evidence an agent needs in order to act competently, and the verification burden created when that context arrives incomplete, stale, or lossily compressed. \textcite{Liu2026} formalises this as \emph{contextual transaction cost}, and Chapter~\ref{ch:coordination-economics} develops the full implications for how agent collectives should --- and should not --- be organised.
