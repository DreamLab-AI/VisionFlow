...
Part~\ref{part:problem} diagnosed the condition: a coordination function passing to machines faster than the institutions around it can adjust, a frontier whose edges stay jagged, a vigilance that decays as the tools improve, and a transition that will be managed or merely endured. Part~\ref{part:solution-space} turns from diagnosis to response. Before offering its own answer, it is worth seeing how the field has already tried to answer, because the serious proposals now come from the consultancies advising the firms that must decide rather than from the academy. Three models have emerged, and they disagree on the question that matters most: how much human coordination the system keeps.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{nb-three-models.jpg}
\caption{The three models compared: Block's Intelligence Layer, Every's Agent Colony, and DreamLab's Dynamic Agentic Mesh, positioned by how much human coordination each retains. Redrawn in the house style from the book's diagram sources.}
...
\textcite{Spicer2026} observes that ``every wave of office technology produces delayering that subsequently reverses as complexity reasserts itself.'' The Klarna cautionary tale is instructive: the CEO claimed AI was performing the work of 700 customer service agents, but customer satisfaction dropped sharply, and the CEO admitted that ``cost unfortunately seems to have been a too predominant evaluation factor'' \parencite{Klarna2025}. Klarna's position has since evolved into an explicitly hybrid AI-plus-human model, with gig-staffed human support handling the interactions the AI could not \parencite{KlarnaHybrid2026} --- the reversal was real, but the end state is recalibration, not retreat. An Orgvue/Forrester survey found that 55 per cent of companies that rushed to replace humans with AI now regret the decision \parencite{OrgvueForrester2025}.
...
\subsection{A Further Data Point: Replit's Self-Driving Company}

Replit's ``The Self-Driving Company'' \parencite{MasadKennedy2026}, published by Amjad Masad and Scott Kennedy on 16 July 2026, is the same genre of primary source as Block's and pushes the pattern one step on: where Block proposes eliminating middle managers, Replit reports agents already running code review, incident triage, business analysis and support across a single firm. Its sharpest figures are a claimed 5.8-times growth in lines of code contributed between early January and late June 2026, which the post itself concedes conflates a roughly doubled engineering headcount; a cohort-consistent 2.9 times at constant authorship, offered as the adjusted number; and 30 per cent of human review time saved, with the hardest support tickets closed 60 per cent faster. Every figure is the vendor's own internal telemetry, published with no raw data, no denominators, no methodology appendix and no external audit, and each is self-reported by the company that also sells the agent being credited.

Read structurally, the post independently restates this book's thesis mechanics: goals are set by people, agents gather context and perform the work, results are checked, judgment-gated escalation routes the hard cases to humans, and manager-agents spawn worker loops beneath them. The convergence with the mesh is on the diagnosis, that coordination is passing to agents while human judgment is retained at the boundary; it is agreement on the problem statement rather than evidence for any particular architecture. Replit's post does not validate VisionFlow, and this book does not launder its numbers into support.

The divergence is the trust model. All of it runs inside one company's ZeroTrust perimeter, where identity, audit logging and escalation are employer-owned controls that hold because a single operator both sides already trust issues and revokes them. The post's own closing section concedes that the next step, letting agents act beyond that perimeter, first needs ``policy, permissions, security, and cost controls'' it does not yet have. That boundary is where this book's four guarantees begin, because employer-owned controls do not compose across organisations: verifiable identity, disclosed agency, human-held escalation and receipts an adversary can check are what a shared operator supplies for free inside one firm and what nothing supplies between two. The measurement invites the same scepticism Block received. Lines of code is a weak productivity proxy, ``quality held'' is self-graded, and the flat-incident and flat-reversion claims are trend statements over a window in which headcount, workload and tooling all changed at once, offered without an exposure base. \textcite{METR2025DevRCT}'s finding that experienced developers misjudge the direction of their own AI-assisted productivity is the standing reason to treat a firm's self-assessment as a hypothesis awaiting audit, not a settled result.

\begin{cautionbox}
Replit converges with this book on the diagnosis and diverges on the trust model. Its self-reported, unaudited gains hold only inside one employer's perimeter, and the identity, escalation and audit controls that make agent operations safe there are precisely the ones that do not travel between organisations.
\end{cautionbox}
...
\textbf{Error multiplication.} Errors in one agent's output propagate and amplify through downstream agents. The first edition cited a community-sourced 17-times multiplication figure \parencite{TDS2026}; the peer-reviewed grounding is now stronger and starker; \textcite{Cemri2025} document failure rates of 41 to 86.7 per cent across seven state-of-the-art open-source multi-agent frameworks, with inter-agent misalignment one of the three principal failure categories (Chapter~\ref{ch:coordination-economics}).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{nb-three-layer-mesh.jpg}
\caption{DreamLab's Dynamic Agentic Mesh: Discovery Engine, Orchestration Layer, and Declarative Governance, with the judgment broker threaded through all three layers. Redrawn in the house style.}
...
\addlinespace
Primary risk & Falling asleep at the wheel & Correlated multi-agent failure at scale (41--86.7\% in naive frameworks) & Unproven beyond 15-person team \\
\bottomrule
\end{tabularx}
\end{table}
...
Until now, this comparison has rested on case evidence and the jagged frontier alone. \textcite{Liu2026}'s formal treatment of agent-collective organisation, developed in Chapter~\ref{ch:coordination-economics}, gives the three models a fourth lens: Block's Intelligence Layer maps onto the hierarchy-manager form, which underperforms a single competent agent on both efficiency and quality; Every's agent colony risks the committee-debate dynamic that Liu finds has an optimal scale of one agent, unless its public channels are deliberately kept closer to structured blackboard communication than free-text deliberation; and the Dynamic Agentic Mesh maps onto the combination Liu's simulation ranks highest --- blackboard memory and adaptive meta-organisation, governed by an interface organisation.
