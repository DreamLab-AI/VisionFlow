...
The medium also makes the signed record the object of coordination rather than a by-product of it. A request for a decision, the decision itself, and every later supersession are events with the same shape and the same signature discipline, so the trail a regulator or a teammate would want to reconstruct already exists as data the moment the decision is made. That is the strong form of the claim, and the gap-close sprint moved its honest weakness a long way. The panel a human signs against now carries a reasoning trace, a confidence figure, and a risk tier alongside the title, description, and action buttons (\texttt{0b8a1c}), and the decision history the relay writes to \texttt{broker\_decisions} is readable at last through a \texttt{GET /api/governance/decisions} endpoint. The audit substrate was always present; the human-facing view into it, at the moment it would most help a person decide well, is now wired rather than pending.
...
Two honest limits sat under the polished parts of the April survey, and the gap-close sprint closed the reachable part of both. First, the richer case lifecycle the documentation advertised was not reachable: \texttt{nostr-bbs-core} defined a fully unit-tested \texttt{DecisionOrchestrator} with delegate, promote and precedent transitions that nothing outside its own test module called, so the live projection understood only \texttt{approve} to resolved and \texttt{reject} to rejected. The sprint wired that orchestrator into the \texttt{kind-31403} projection (\texttt{6986276}), so delegate, promote and precedent now resolve a case rather than parking it in \texttt{under\_review}, and a risk tier rides each decision with the low-tier Memo suppressed. Second, and more consequential for the ecosystem's framing, the shipping forum client gated the entire \texttt{/governance} route to admins; \texttt{AdminGatedGovernance} bounced any authenticated non-admin before the page mounted, which contradicted the document's Trust and Gating table granting ordinary members a panel view. The sprint split the route (\texttt{0b8a1c}): an ordinary member now reaches a read-only governance view, while signing stays admin-gated at the relay. The kit still stops short of the universal human-in-the-loop plane the README advertises, so the flagship chokepoint remains closer to ``forum, then an admin decides'' than to distributed judgment, but members are no longer shut out of the room entirely. Chapter~\ref{ch:gap-register} tracks the residual distance.

A subtler provenance defect compounded the first, and the sprint closed it too. The forum client used to publish the signed \texttt{kind-31403} fire-and-forget and flip the card to ``Response sent'' the instant local signing succeeded, without waiting for the relay's NIP-01 OK frame, so an operator could believe they governed an action when nothing was accepted. It now signs through \texttt{publish\_with\_ack} at both response sites (\texttt{0b8a1c}), the same relay-confirmed pattern the retro BBS client already used, surfacing the rejection reason inline; on a governance surface, confirming a decision the relay refused was a trust-calibration failure, not a cosmetic one, which is why the acknowledged path was P1 work.
...
The write path itself is the honest part. \texttt{ADR-105} originally decided the BBS would never hold key material and would delegate signing to the forum through a same-origin \texttt{sign()} seam. The shipped M2 write path instead holds a \texttt{Keypair} directly through the kit's NCC-audited \texttt{PrfSigner}, and the ADR's 2026-07-03 amendment records the change in place. Because the two clients are served same-origin, any script that could reach a \texttt{sign()} bridge can already read the session key, so a direct hold and a delegated seam sit on the identical trust boundary, and the in-memory path fails closed when no signer is present. The amendment also flags, rather than letting the divergence pass silently, the residual boundary it did not close: a passkey user who generates a fresh in-memory key signs under a distinct key, not the PRF-derived root. The retro client's governance action button signs and publishes its \texttt{kind-31403} through \texttt{publish\_with\_ack}, the relay-confirmed pattern the forum client has since adopted at both its response sites (\texttt{0b8a1c}).

\section{One Use Case, Stated Plainly}

The README describes a universal human-in-the-loop control plane for any agent system. The distance between that phrase and current use should be stated without softening. Today the governance plane serves exactly one use case: ontology concept elevation, capped at five concurrent cases, decided on the forum web page by a single administrator role. Much of what the protocol makes possible, from multiple agent systems and multiple communities to fully distributed member judgment, is still scaffolded or unbuilt, but the sprint narrowed the list: graduated decisions richer than approve-or-reject now project through the relay (\texttt{6986276}), and an ordinary member now reaches a read-only view of the queue (\texttt{0b8a1c}). Roster administration moved onto the surface as well, an Agents admin tab (\texttt{0b8a1c}) replacing part of what had been a \texttt{curl}-only chore, though provisioning and revoking the agents that may act remains an admin operation, so the most authority-laden step in the design is now partly on the product surface rather than wholly outside it.

\begin{cautionbox}[title={What the governance plane does and does not yet do}]
\begin{itemize}
  \item Ordinary members can now read the governance queue but still cannot sign a decision; the route is read-only for non-admins (\texttt{0b8a1c}), so distributed human judgment is a viewing right, not yet a voting one.
  \item The delegate, promote and precedent lifecycle now projects through the relay with a risk tier attached (\texttt{6986276}); what remains unbuilt is the multi-party resolution of conflicting decisions, not the transitions themselves.
  \item A published decision is immutable by design; a supersession path now exists in scaffolded, canon-gated form, where an authorised signer or higher supersedes a prior \texttt{kind-31403} by a new signed reference and the projection marks the earlier one \texttt{Superseded} (\texttt{35dbb1b}). Appeal by an ordinary member who would contest a call is still unspecified.
  \item Agents now carry a disclosure badge at every author-render site (\texttt{7157a92}); what stays thin is the display of the exact principal that authorised each agent.
