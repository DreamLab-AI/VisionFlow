...
The security of that path is a live gap, and the freshest ground truth says so plainly. As
of the 3 July 2026 closeout, the \texttt{propose} endpoint still
accepts unauthenticated proposals and a \texttt{/api/ontology/load} backdoor still bypasses
Whelk entirely, which is why the first recommendation of
Chapter~\ref{ch:ecosystem-roadmap} is to close both before anything downstream is worth
measuring. The authentication fix (\texttt{ADR-120}) is written but not yet landed and
verified on the Rust side. A governance pipeline with an unauthenticated front door is a
design for governance, and Chapter~\ref{ch:gap-register} records it as such.
...
(\texttt{ADR-119}): a green-but-zero dashboard is treated as failure. The three read
channels are built and verified in the agentbox Node layer (eighteen of eighteen unit tests
passing), while the server-side workstreams (the SPARQL row clamp, the propose
authentication, the fenced derived-graph endpoint) were written pending the Rust host
build. In the governed vocabulary of \texttt{ADR-002}, the binding is integrated on its
read path and still hardening on its write path.
...
concept out of an individual's knowledge graph into the shared ontology) currently skips
the Whelk consistency gate (finding GOV-7, open at the 3 July closeout). A concept can
therefore reach the human-policy gate without first clearing the correctness gate the canon
places ahead of it. The espoused sequence and the wired sequence disagree on exactly one of
the two elevation routes, and the register records the disagreement rather than smoothing
it over.
