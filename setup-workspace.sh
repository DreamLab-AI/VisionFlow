...
# 7. Simplified MCP Server Setup (Copy-based, no patching)
setup_mcp_server() {
    log_info "🔧 Setting up MCP server using pre-fixed files..."

    # 1. Clean up npx cache to prevent version conflicts
    log_info "🧹 Cleaning up npx cache to prevent version conflicts..."
    if [ -d "/home/ubuntu/.npm/_npx" ]; then
        if dry_run_log "Would clean npx cache"; then :; else
            find /home/ubuntu/.npm/_npx -name "*claude-flow*" -type d -exec rm -rf {} + 2>/dev/null || true
            log_success "Cleaned npx cache"
        fi
    fi
    if [ -d "/home/dev/.npm/_npx" ]; then
        if dry_run_log "Would clean dev user npx cache"; then :; else
            find /home/dev/.npm/_npx -name "*claude-flow*" -type d -exec rm -rf {} + 2>/dev/null || true
            log_success "Cleaned dev user npx cache"
        fi
    fi

    # 2. Install/Verify claude-flow
    if ! command -v claude-flow >/dev/null 2>&1; then
        ...
        if dry_run_log "Would install claude-flow"; then :; else
            npm install -g claude-flow@alpha --force || {
                ...
                sudo npm install -g claude-flow@alpha --force || {
    ...
    # 3. Find claude-flow installation path
    ...
    # 4. Copy pre-fixed mcp-server.js (no patching, just copy)
    if [ -f "$mcp_server_src" ]; then
        ...
        log_warning "Pre-fixed mcp-server.js not found at $mcp_server_src (will use default)"
    fi

    # 5. Verify mcp-tcp-server.js is correct
    local tcp_server_path="/app/core-assets/scripts/mcp-tcp-server.js"
    if [ -f "$tcp_server_path" ]; then
        # Check if it uses the global binary
        if grep -q '"/usr/bin/claude-flow"' "$tcp_server_path" 2>/dev/null; then
            log_success "mcp-tcp-server.js is using global claude-flow binary"
        else
            log_warning "mcp-tcp-server.js may not be using global binary"
        fi
    fi
    ...
    if dry_run_log "Would create /workspace/.swarm and set CLAUDE_FLOW_DB_PATH"; then :; else
        ...
        echo "export CLAUDE_FLOW_GLOBAL=/usr/bin/claude-flow" >> "$profile_file"
        chmod +x "$profile_file"
        log_success "Configured shared database path in $profile_file"
    fi

    # 7. Create npx wrapper to prevent downloading new versions
    log_info "Creating npx wrapper to force global version..."
    if dry_run_log "Would create npx wrapper"; then :; else
...
EOF
        ...
        log_success "Created npx wrapper"
    fi

    # 8. Restart MCP TCP server
    ...
    if echo "$test_result" | grep -q '"id":"agent-1"'; then
        log_warning "MCP server might still be returning mock data (agent-1 found)"
        log_info "This will be fixed after container restart with updated files"
    elif [ -z "$test_result" ]; then
        log_warning "Could not connect to MCP TCP server on port 9500 (may need restart)"
    else
        log_success "MCP server is responding (mock data check passed)"
    fi
}
...
show_setup_summary() {
    ...
    echo "  - Cleaned npx cache to prevent version conflicts"
    echo "  - Set up MCP server with fixed files (no mock data)"
    echo ""
