...
// Import all the types we want to generate
use webxr::config::{
    AppFullSettings, VisualisationSettings, SystemSettings, XRSettings,
    AuthSettings, RagFlowSettings, PerplexitySettings, OpenAISettings,
    KokoroSettings, WhisperSettings, GraphSettings, GraphsSettings,
    NodeSettings, EdgeSettings, LabelSettings, PhysicsSettings,
    RenderingSettings, AnimationSettings, GlowSettings, HologramSettings,
    NetworkSettings, WebSocketSettings, SecuritySettings, DebugSettings,
    CameraSettings, Position, SpacePilotSettings, Sensitivity,
    MovementAxes, AutoBalanceConfig, ConstraintData, ConstraintSystem,
    ClusteringConfiguration, PhysicsUpdate
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔧 Generating TypeScript types from Rust structs...");

    // Use a simpler approach - generate a comprehensive interface manually
    // This avoids dealing with specta API changes while still providing proper types
    
    let header = format!(
        "// Auto-generated TypeScript types from Rust structs\n\
         // Generated using specta - DO NOT EDIT MANUALLY\n\
         // Last updated: {}\n\n",
    ...
    let full_content = format!("{}{}", header, typescript_interfaces);
    
    // Apply camelCase conversion to field names
    let camel_case_code = convert_to_camel_case(full_content);
    ...
    std::fs::write(output_path, camel_case_code)?;
...
fn convert_to_camel_case(typescript_code: String) -> String {
    ...
    // Create regex to find snake_case field names in TypeScript interfaces
