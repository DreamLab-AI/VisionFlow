...
/// Generate TypeScript types using the generate-types binary
fn generate_typescript_types() {
    use std::process::Command;
    
    // Run the generate-types binary to create proper TypeScript interfaces
    ...
    match output {
        Ok(result) => {
            if result.status.success() {
                println!("cargo:warning=Successfully ran TypeScript type generation");
            } else {
                println!("cargo:warning=TypeScript type generation failed with status: {}", result.status);
                // Print stderr if available for debugging
                if !result.stderr.is_empty() {
                    println!("cargo:warning=Type generation error: {}", String::from_utf8_lossy(&result.stderr));
                }
            }
        }
        Err(e) => {
            println!("cargo:warning=Failed to execute type generation: {}", e);
        }
    }
    
    // Mark for rebuild when config changes
    println!("cargo:rerun-if-changed=src/config/mod.rs");
    println!("cargo:rerun-if-changed=src/bin/generate-types.rs");
}