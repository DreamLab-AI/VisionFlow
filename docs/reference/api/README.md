...
```typescript
const ws = new WebSocket('ws://localhost:9090/ws?token=YOUR_JWT');
...
def get_hierarchy(ontology_id="default", max_depth=None):
...
root_classes = hierarchy["rootClasses"]

for class_iri in root_classes:
    node = hierarchy["hierarchy"][class_iri]
...
- `github_sync_state` - Sync tracking
- `workspaces` - User workspaces
- `validation_reports` - Ontology validation
