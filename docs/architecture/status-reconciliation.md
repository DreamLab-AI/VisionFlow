...
| Embodied agent loop (`/wss/agent-events`) | **Phase 2a wired and cargo-verified (2026-05-29).** agentbox emits canonical `notifications/agent_action` (identity preserved per ADR-013) via one builder; VisionClaw authenticates the WS upgrade, validates, and publishes to a process-global broadcast hub (7/7 tests). Open: beam+gluon render (ADR-059 Phase 2b), live did:nostr-keyed actor nodes, `ConceptElevated`, `:9500` state-poll cutover. Contract: agentbox ADR-014 + VisionClaw ADR-059. |
