...
<a id="augmentation-conditions--graded-2026-09-14-adr-2010"></a>

## Augmentation conditions — graded 2026-09-15 (ADR-2010)

The six conditions of arXiv 2609.12482 grade every surface where a human decides on an agent's behalf. Status vocabulary: `absent` (no code or receipt), `partial` (mechanism exists but the condition is not measurable from it), `measured` (a runtime quantity or receipt exists). A status may be raised only by a `file:line` citation to code or a receipt, never by a document. Citations are to the merged `main` heads of the three substrates — nostr-rust-forum `11b674c`, agentbox `b859b37d2`, VisionClaw `8f9affe49` — where PRD-augmentation-conditions FR2–FR7 landed. C1–C3 are snapshot conditions; **C4–C6 are longitudinal, so a first `measured` status on those rows is a baseline, not a pass (ADR-2010 consequence 3) — it is this cycle's only measurement, and a second cycle (review_trigger: 2026-12-14) is what actually grades whether learning, pathways and job purpose are holding up over time.**

| Condition | nostr-rust-forum | agentbox | VisionClaw |
|---|---|---|---|
| C1 Durable net value | `partial` — per-reviewer time-to-decision is now measured (`crates/nostr-bbs-auth-worker/src/governance_api.rs:1287-1288`) but nothing forum-side differences it against a productivity claim | `partial` — dream-ledger reviewer minutes are measured per reviewer (`management-api/lib/dream-ledger.js:203-222`) but no net-value calculation consumes them | `measured` — HITL Precision is a real computation over decided cases, no longer a stub (`src/services/kpi_compute.rs:213`, wired at `src/services/kpi_compute.rs:408-420`); Whelk-gate rows are excluded from the human-only series (`src/services/kpi_compute.rs:140-154`) |
| C2 Meaningful human control | `measured` — the proposal `fields`/reasoning render below the controls and the typed rationale is what publishes, never a template (`crates/nostr-bbs-forum-client/src/utils/governance_view.rs:57,77,445`); the reviewer-role admission gate gives delegated authority to override (`crates/nostr-bbs-relay-worker/src/relay_do/nip_handlers.rs:234-249`) | `measured` — the fail-closed deny path names the manual-continuation tool at the point of denial, the code comment cites C2 by id (`management-api/lib/authority.js:395-404`); tightening is a lattice join the operator's manifest always wins (`management-api/lib/authority.js:122-140`) | `measured` — the proposal renders unclipped and the rationale gate is enforced server-side, not just in the client (`client/src/features/control-center/governance/AcspCaseQueue.tsx:16-24`; `src/handlers/enrichment_proposals_handler.rs:406-423,503-518`) |
| C3 Accountability and recovery | `measured` — the application-receipt endpoint advances `broker_decisions` through `consumer-received/applied/not-applied` (`crates/nostr-bbs-auth-worker/src/governance_api.rs:1124`); the ageing cron mints `escalated-on-age` receipts past `max_pending_hours` (`crates/nostr-bbs-relay-worker/src/cron.rs:595`, wired at `crates/nostr-bbs-relay-worker/src/lib.rs:1003`); the client renders every ladder/terminal/conflicting stage (`crates/nostr-bbs-forum-client/src/stores/receipts.rs:56-86`) | `measured` — every gate denial is appended to the hash-chained authority journal with stage and reason (`management-api/lib/authority-journal.js:42,92-115`); receipt-post failures are journalled, never silent (`management-api/lib/governance-receipt-publisher.js:193-206,312`) | `measured` — `ElevationActor` gains a 14-day `OPEN_CASE_TTL`, a `Reconcile` message and an `expired` receipt path (`src/actors/elevation_actor.rs:68,93,1099-1132`) |
| C4 Deepening learning (baseline) | `measured` (baseline) — `GET /api/governance/reviewers` computes median/p90 time-to-decision, override rate and calibration/probe catch rate per reviewer; the code names C4 and C6 as the questions it answers (`crates/nostr-bbs-auth-worker/src/governance_api.rs:1336-1394,1397`) | `measured` (baseline) — dream-ledger `reviewerStats` tracks review count and median review-minutes per reviewer across nights, the first cross-night reviewer signal in the estate (`management-api/lib/dream-ledger.js:203-222`) | `absent` — PR outcome still feeds only a status column, no VisionClaw-owned human-learning measure exists (`src/actors/elevation_actor.rs:1131-1200` unchanged from the prior grade) |
| C5 Career pathways (baseline) | `measured` (baseline) — a `reviewer` role now exists and an admin's `Delegate{to}` admits that reviewer's 31403 for exactly the delegated case, giving juniors substantive (not all-or-nothing) review work (`crates/nostr-bbs-relay-worker/src/relay_do/nip_handlers.rs:203-249`, delegation recorded at `:2325`) | `absent` — no per-reviewer work-allocation mechanism; dream-ledger records who reviewed but does not assign substantive work | `absent` — no reviewer-role or delegation mechanism in this substrate; owned by the forum per the field-ownership note below |
| C6 Job purpose (baseline) | `measured` (baseline) — the published `reasoning` is the human's own typed text, gated by `check_rationale`/`MIN_HUMAN_RATIONALE_CHARS`, and `decision_content` never fabricates it (`crates/nostr-bbs-core/src/governance.rs:711-712,761`; `crates/nostr-bbs-forum-client/src/utils/governance_view.rs:77`); `"Human {action} via governance UI"` is gone from the codebase (only a removal comment remains, `crates/nostr-bbs-forum-client/src/utils/governance_view.rs:624`) | `measured` (baseline) — a manual continuation requires the operator's own words as `evidence`, refused when absent (`management-api/lib/governance-manual-continue.js:82,105,122-123`) | `measured` (baseline) — the client rationale gate and server-side `check_rationale` both require the reviewer's typed text; the fabricated-string path is deleted (`client/src/features/control-center/governance/AcspCaseQueue.tsx:16-24`; `src/handlers/enrichment_proposals_handler.rs:406-423`) |

Known limit carried forward from grading, not closed by this merge: the seeded-probe catch rate (feeding forum C4) rides the raw signed 31402, so the measured agent can see its own probe tag and answer it correctly — this is ADR-2011's own `review_trigger`, tracked as forum `docs/security/known-findings.md` KF-10 (`crates/nostr-bbs-core/src/governance.rs:343,365,596`). It does not by itself demote C4 from `measured`, because the metric exists and runs; it does mean the catch-rate number the metric reports is a floor, not the true rate, until the tagless design in the DDD supersedes it.

Workflow-record field ownership (ADR-2010 §3): agent authority level — agentbox `authority_class` (`agentbox.toml:822-849`); review/override/escalation points — forum gates (`nip_handlers.rs:715-763`); fallback/recovery — agentbox authority gate + forum receipts; **unapproved agent actions — agentbox authority-journal** (`management-api/lib/authority-journal.js:42,92-115` — every `authority.deny` is a hash-chained record of an action the gate refused to let proceed); **effort including verification — forum reviewers telemetry** (`crates/nostr-bbs-auth-worker/src/governance_api.rs:1287-1288,1336-1394` — median/p90 time-to-decision and override rate per reviewer); **effects on human capability — agentbox dream-ledger reviewer columns + forum reviewers telemetry** (`management-api/lib/dream-ledger.js:203-222`; `crates/nostr-bbs-auth-worker/src/governance_api.rs:1336-1394` — review-minutes-per-night and calibration/probe catch rate over time are the two cross-cycle capability signals the estate now has). All three prior matrix defects have an owner as of this grading.

Branch state: the three `feat/augmentation-conditions` branches merged to `main` — nostr-rust-forum `11b674c`, agentbox `b859b37d2`, VisionClaw `8f9affe49` — closing the milestone tracked in PRD-augmentation-conditions §Milestones (M2). Re-grade on the next substrate-branch merge that touches a cited path, or at the 2026-12-14 longitudinal review trigger, whichever comes first.
