...
| Q15 | Site root | **Corrected by the owner 2026-09-23:** the Narrative Goldmine explorer (React/WASM) stays at `/`, reading the new `vault build` data; Quartz renders the Obsidian vaults at `/notes/` only, replacing the Logseq notes app there. (Recorded 2026-09-22 as "Quartz owns `/`, explorer at `/explorer/`", chosen on the agent's recommendation after a Q13 framing that presented Quartz as replacing the whole site. It was deployed for one day; the owner reversed it as an error.) |
| Q13 | Publisher | Quartz v4 renders the research notes at `/notes/` (ExplicitPublish ⇐ `public: true`), replacing the Logseq notes app. `vault build` emits the machine artefacts (`api/*.json` including the markdown mirror, `ontology.ttl`, graph tiers, OKF bundle, JSON-LD context) at the site root, where the explorer reads them. (Scope corrected 2026-09-23 with Q15.) Owner pushes; agents never push. |
...
6. narrativegoldmine.com builds locally in its corrected layout (Q15, 2026-09-23): the explorer at `/` reading the `vault build` artefacts (every search-index entry has its `/api/pages/<id>.json` and `/api/markdown/<title>.md`); Quartz's notes complete under `/notes/` (staged pages == built pages) with nothing of Quartz at the root; `/explorer/` redirects to `/`. Verified by `quartz/scripts/check-urls.sh`.
