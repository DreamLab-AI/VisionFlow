...
2. **Semantic degradation baseline (PROMOTED):** ingest a known corpus into AgentDB, measure the distance matrix under hash-placeholder embeddings. Quantifies how degraded RuVector memory recall is under the crate default; sets the acceptance bar for the default-path embedding fix (the live deployment already embeds via `bge-small-en-v1.5`, not MiniLM).
