...
VisionFlow is not a single executable. It is a coordination architecture that emerges from six product substrates and one branded deployment:
...
| agentbox | Reproducible sovereign agent runtime with Nix, 130 skills, 180+ MCP tools, 12-tool ontology bridge to VisionClaw SPARQL, browser setup wizard, privacy filtering, Nostr/Solid identity | Agent container and harness |
...
| loom (Ontology Loom) | Grounding node: model-swappable façade serving the reasoned ontology into any model's context at query time (`/loom/scaffold` + OpenAI-compatible chat); the context-assembly layer | Serving façade; the model behind it is a URL, swapped by benchmark |
...
**Augmentation-conditions extension (ADR-2010/ADR-2011; on branch `feat/augmentation-conditions`, not merged, not deployed).** Three substrates carry a substrate branch implementing the six augmentation conditions as the audit lens for this loop (see [PRD-augmentation-conditions](PRD-augmentation-conditions.md)):
   - **Task-property triple and effective tier.** `nostr-bbs-core` gains `TaskProperties { verifiability, reversibility, stakes }` (nostr-rust-forum `crates/nostr-bbs-core/src/governance.rs:391-397`, `feat/augmentation-conditions` @ `aa438f3`), carried as `tp-verifiability`/`tp-reversibility`/`tp-stakes` tags on the panel (31400, tightening-only on 31402). The pure function `effective_tier(panel, request, declared, advertised_default)` (`governance.rs:516-534`, same branch) is the only tier stored on `broker_cases.effective_tier` or read for suppression/routing — the requesting agent's own `risk_tier` is telemetry-only from here on. agentbox derives the panel default from `authority_class` (`zero-tolerance ⇒ Irreversible`) in `management-api/lib/task-properties.js:100-113` (agentbox `feat/augmentation-conditions` @ `19463a588`).
   - **Application receipts and delegation.** The forum auth worker exposes `POST /api/governance/receipts/{response_event_id}/application` accepting `consumer-received | applied | not-applied | applied-manually` (nostr-rust-forum `crates/nostr-bbs-auth-worker/src/governance_api.rs:1104-1111`, `aa438f3`); agentbox's `governance-receipt-publisher.js:154-344` (agentbox `19463a588`) posts these after `ApplicationReceiptStore.begin`/`finish`, journalling any post failure rather than swallowing it (`authority-journal.js:69-184`).
   - **Reviewer telemetry.** `GET /api/governance/reviewers` (nostr-rust-forum `governance_api.rs:1374-1387`, `aa438f3`) is the read model over `broker_decisions × broker_cases × governance_receipts` used to measure C4/C6.
   - **Manual continuation.** agentbox MCP tool `governance_manual_continue` (`mcp/servers/governance-bridge.js:218-237`, `19463a588`; implementation `management-api/lib/governance-manual-continue.js:53-256`) records an operator-executed action as an `applied-manually` receipt bound to a prior `Approve`; the authority gate's `no-decision-surface` deny path (`management-api/lib/authority.js:302-304`, `19463a588`) names the tool as a structured hint at the moment of denial.
   - **Intent persistence and HITL Precision.** VisionClaw persists the envelope's declared `intent` and computes `intent_match` (`src/services/intent_match.rs:96-162`, VisionClaw `feat/augmentation-conditions` @ `f2b9f803e`); `kpi_compute.rs::hitl_precision()` (`src/services/kpi_compute.rs:213-227`, same branch) is computed from decided cases (warranted ÷ decided) rather than the `awaiting_data_source` stub.
   - **Non-vacuous rationale capture.** VisionClaw's `AcspCaseQueue.tsx` now renders the proposal and requires a typed rationale (20+ characters) on `high`/`critical` tiers before the decide button unlocks (`client/src/features/control-center/governance/AcspCaseQueue.tsx:62-185`, `f2b9f803e`), and the server-side gate `check_rationale()` (`src/handlers/enrichment_proposals_handler.rs:406-419`, `f2b9f803e`) rejects a `high`/`critical` decision made without one.
...
- **Augmentation-conditions substrate work — branch state only, not merged (2026-09-14).** All three governance substrates carry a `feat/augmentation-conditions` branch implementing FR3–FR7 of [PRD-augmentation-conditions](PRD-augmentation-conditions.md); none is merged to main or deployed. nostr-rust-forum @ `aa438f3` (`WIP-STATUS-augmentation-conditions.md`: deepsec-gate is BLOCK not PASS, 2 pre-existing HIGH findings untriaged, `activation_status: inactive`; forum-client rendering of the task-property triple, `effective_tier`, and probe-tag hiding is explicitly **not done** on this branch). agentbox @ `19463a588` (task-property derivation, receipt publisher, manual continuation, authority-deny journal, `dream-ledger.js` `Reviewer`/`Review-minutes` columns — `management-api/lib/dream-ledger.js:54-212`). VisionClaw @ `f2b9f803e` (intent persistence, HITL Precision, `ElevationActor` `OPEN_CASE_TTL`/reconciliation — `src/actors/elevation_actor.rs:68-72,1103-1164` — and the rationale gate on `AcspCaseQueue`). Re-grade the [compatibility matrix](architecture/compatibility-matrix.md) augmentation-conditions rows only on merge (ADR-2010 §Verification).
