...
```mermaid
flowchart TB
    DD1["README.md:383 calls NIP-42 AUTH scaffolded with auth_required false. The code says otherwise:<br/>nip42 is the DEFAULT nip42.rs:62, the template ships AUTH_MODE nip42 nostr-bbs-relay-worker/wrangler.toml:31,<br/>and every EVENT passes the gate first nip_handlers.rs:514. The row understates a shipped feature.<br/>See NF-03.3."]
...
flowchart TB
    ...
    S5["MESH_ALLOWED_REMOTE_DIDS ships empty nostr-bbs-relay-worker/wrangler.toml:55, so the federated-kind gate at<br/>nip_handlers.rs:563 is inert. Standalone is the only supported mode. See NF-03.13."]
...
flowchart LR
    ...
    Q2006["Current sweep: TL2 may move directly to TL0; stable keyset paging and explicit<br/>commit/error outcomes replace OFFSET and ignored writes. D1 batches state and audit.<br/>Remaining: an optimistic conflict can still commit a false audit row; see NF-10.10."]
    ...
    N1["DOC-DRIFT: the IDENTITY-keys-and-trust closeout still states OFFSET row-skipping and ignored write<br/>errors as CURRENT defects. Both are fixed - keyset paging nostr-bbs-relay-worker/src/trust_sweep.rs:432<br/>and confirmed-commit-only counters nostr-bbs-relay-worker/src/trust_sweep.rs:462. The governing doc needs<br/>the qualification retired; see NF-11.11 and NF-11.12."]
    N2["DOC-DRIFT: the relay-side receipt machine is REAL, not proposed - stages at<br/>nostr-bbs-relay-worker/src/relay_do/receipts.rs:76, applied at receipts.rs:316, and handle_event logs<br/>accepted-but-not-applied from the returned receipt nip_handlers.rs:926. Only the CONSUMER half is absent.<br/>See NF-11.9 and NF-11.10."]
...
## NF-10.10 Optimistic trust conflict — a successful transaction can contain a no-op
...
sequenceDiagram
    ...
    S->>S: Read candidate at TL1
    Note over S,D: Concurrent authority change sets stored row to TL3
    S->>D: UPDATE whitelist WHERE pubkey and trust_level = TL1
    D->>D: UPDATE succeeds with zero affected rows
    S->>D: Unconditional INSERT admin_log claims TL1 to TL0
    D->>D: Commit batch successfully
    D-->>O: meta.changes = 0
    O->>O: Report conflict and failed count
    Note over D,O: Actual TL3 survives, but the audit row already committed.<br/>Checking changes after commit cannot retract the false audit.<br/>guarded UPDATE trust_sweep.rs:432, unconditional audit INSERT trust_sweep.rs:442,<br/>single batch trust_sweep.rs:462 - SQLite source-query probe, not a live D1 incident.
```

The native relay suite passes 239 tests. Those tests do not discharge this adapter-level zero-row case. Acceptance requires the audit INSERT to depend on the guarded UPDATE in the transaction, plus tests of concurrent level, exemption and activity changes. Historical OFFSET skipping is fixed and must not remain a current finding.
