...
sequenceDiagram
    ...
    alt VAULT_ROOT empty (:102)
        ...
        E->>E: export VAULT_ROOT/VAULT_PAGES/VAULT_FORMAT/VAULT_TUI/VAULT_WORKING_ROOT/VAULT_TRANSCRIPTS AGENTBOX_VAULT_ENABLED=1 (:126-139)
        E->>E: export ONTOLOGY_PAGES_DIR default to VAULT_PAGES (:148)
    end
    E->>E: _ab_cargo_bin_on_path() ADR-2029 D4 (:250)
    E-->>D: echo "[1/8] Preparing runtime directories..." (:252)
    E->>FS: mkdir -p WORKSPACE, RUVECTOR_DATA_DIR, SOLID_POD_ROOT, /run/secrets... (:253-264)
    E->>FS: chmod 0700 /run/secrets, chown 1000:1000 (:270-271)
    E->>FS: chown 1000:1000 volume roots, non-recursive (:301-335)
    E-->>D: echo "[2/8] Bootstrapping sovereign mesh identity..." (:415)
    E->>NPB: nostr-pod-bridge bootstrap (:428)
    ...
    E-->>D: echo "[3/8] Ensuring workspace defaults..." (:426)
    E->>FS: mkdir WORKSPACE/agents if absent (:434-436)
    E->>FS: ln -sf /home/devuser/.claude WORKSPACE/.claude (:440-442)
    opt DREAM_CMD_SRC exists and no dream.md (:448)
        E->>FS: cp dream.md to /home/devuser/.claude/commands/ (:457)
    end
    opt skill-creator not yet registered (:460)
        E->>E: agentbox-manifest plugin-register --key skill-creator@claude-plugins-official (:461-466)
    end
    opt codex plugin baked and not registered (:485)
        E->>E: agentbox-manifest plugin-register --key codex@openai-codex (:486-491)
    end
```
...
sequenceDiagram
    autonumber
    participant E as entrypoint-unified.sh<br/>agentbox/config/entrypoint-unified.sh:547-549
    participant AM as agentbox-manifest<br/>entrypoint-unified.sh:550
    participant FS as rootfs/tmpfs
    participant SV as supervisord<br/>entrypoint-unified.sh:802
    participant B as [program:bootstrap] Stage B<br/>entrypoint-unified.sh:807

    E-->>E: echo "[4/8] Provisioning agent stacks..." (:549)
    E->>AM: agentbox-manifest provision-stacks (:550)
    E->>FS: chown -R 1000:1000 WORKSPACE/profiles (:559)
    E-->>E: echo "[5/8] Validating runtime closure..." (:566)
    E->>FS: bash validate-artifacts.sh (:567)
    alt validate-artifacts.sh fails (:567)
        E-->>E: fatal BootstrapFailed, exit 1 (:568-570)
    end
    E->>FS: mkdir /run/agentbox /run/agentbox/hooks (:578) — bootstrap-seal writes here later
    E->>FS: mkdir/chown/chmod 0700 ~/.config/agent-of-empires (N-05) (:583-597)
    Note over E: N-05 verify — expect mode 700 owner 1000,<br/>logs N-05-VIOLATION marker, non-fatal (:599-613)
    Note over E,FS: ADR-2040 (2026-09-05) — code-server and jupyter-lab credential<br/>minting now runs here (:617-703), between the N-05 verify and the identity.env<br/>source below. Not re-diagrammed here — see AB-06.7 and AB-07.9
    E->>FS: source /run/agentbox/identity.env into PID1 env (:713-714)
    alt AGENTBOX_BRIDGE_SK set (:725)
        E->>FS: write /run/secrets/nostr.key 0400 devuser, unset AGENTBOX_BRIDGE_SK from env (:724-731)
    end
    Note over E: ADR-2028 D3 — ontology PUSH cache refresh (Phase 5c)
    alt ONTO_PAGES empty (:763)
        E-->>E: echo "[5c/8] ontology PUSH cache refresh skipped ([vault] disabled)" (:764)
    else ONTO_PAGES dir exists and builder present (:765)
        E->>FS: run_as_devuser node ontology-index-build.js (:766-769)
    end
    opt AGENTBOX_TAB0_BRIDGE_SUPERVISED=1 and BRIDGE_TOKEN unset (:783)
        E->>FS: generate/read BRIDGE_TOKEN, write 0600 secrets file (:784-797)
    end
    E-->>E: echo "[5b/8] Starting supervisord..." (:801)
    E->>SV: exec supervisord -c /etc/supervisord.conf -n (:802)
    ...
    B->>B: re-export WORKSPACE/RUVECTOR_DATA_DIR/SOLID_POD_ROOT/AGENTBOX_CONFIG defaults (:814-818)
    alt AGENTBOX_VAULT_ENABLED unset (:822)
        B->>B: _ab_vault_resolve() again (standalone-invocation fallback) (:822)
    end
    B-->>SV: echo "[6/8] Validating pre-packaged service closures..." (:836)
    loop _probe_closure for management-api, mcp, gated toolchains (:853-877)
        B->>FS: test -d node_modules under each closure dir
        alt node_modules missing (:846)
            B-->>SV: fatal MissingArtifactDetected, exit 1 (:846-849)
        end
    end
    B-->>SV: echo "[6/8] Service closures OK." (:880)
```
...
sequenceDiagram
    autonumber
    participant B as [program:bootstrap] Stage B<br/>entrypoint-unified.sh:959
    participant AM as agentbox-manifest<br/>entrypoint-unified.sh:1632
    participant MCP as .mcp.json<br/>entrypoint-unified.sh:1032
    participant FS as rootfs/tmpfs
    participant RT as runtime-env.sh<br/>entrypoint-unified.sh:2700
    participant TS as trust-seed.cjs<br/>registered from entrypoint-unified.sh:1316-1326
    ...
    B-->>B: echo "[7/8] Bootstrapping ruflo plugins..." (:959)
    B->>FS: mkdir ~/.claude-flow/plugins, /var/cache/ruflo-plugins (:964-965)
    opt claude-flow-config.template.json present, config.json stale (:975)
        B->>FS: sed RUVECTOR_PG_PASSWORD into ~/.claude-flow/config.json (:973-974)
    end
    Note over B: PRD-018/ADR-036 D6 — read RuVector memory gate flags<br/>via _ab_toml_bool memory_learning.* (:983-1029), fail-open all-off
    B->>MCP: ensure .mcp.json points at ruvector-mcp.cjs, idempotent (:1032-1066)
    B->>MCP: inject PRD-018 RuVector memory gate env (:1096-1175)
    opt browser-gpu sidecar reachable (:1630)
        B->>AM: agentbox-manifest mcp-set-server --name browser-gpu (:1632)
    end
    B->>AM: agentbox-manifest mcp-reconcile-aqe --provider "$_MR_PROVIDER_ARG" (:1659)
    opt _MR_ENABLED=1 — ADR-041 model routing (:1664)
        B->>AM: run_as_devuser agentbox-manifest model-routing-project --manifest AGENTBOX_CONFIG --workspace WORKSPACE (:1665-1667)
        Note right of AM: projects [model_routing.routes] into every<br/>.agentic-qe/llm-config.json under the workspace
    end
    Note over B: ADR-069 — interaction_plane.proxy projected every boot (:1712-1716)
    opt AGENTBOX_CONFIG exists and agentbox-manifest present (:1713)
        B->>AM: agentbox-manifest nip98-config --manifest AGENTBOX_CONFIG --out .agentbox/nip98-proxy-config.json (:1714)
        B->>FS: chown 1000:1000, chmod 600 nip98-proxy-config.json (:1716-1717)
    end
    opt ENABLE_ONTOLOGY=true and ontology-bridge.js present (:1732)
        B->>AM: agentbox-manifest mcp-set-server --name ontology-bridge (:1733)
    end
    opt AGENTBOX_TRUST_SEED not 0 and node present (:1317)
        B->>TS: node trust-seed.cjs marks workspace root and worktrees trusted (:1318)
        B->>FS: register trust-seed SessionStart hook in settings.json idempotent (:1319-1326)
    end
    B-->>B: echo "[8/8] Publishing environment hints..." (:2696)
    B->>RT: cat RUNTIME_ENV_FILE=/run/agentbox/runtime-env.sh heredoc (:2700-2769)
    ...
    B->>FS: ln -sf RUNTIME_ENV_FILE /etc/profile.d/agentbox-runtime.sh best-effort (:2806-2807)
    B->>FS: cp RUNTIME_ENV_FILE to durable WORKSPACE/.agentbox-runtime-env.sh (:2815)
    B->>FS: write fish conf.d/agentbox-runtime.fish sourcing the env file (:2819-2821)
```

## AB-02.4 supervision tree — core and identity programs
```mermaid
flowchart TB
    BOOT["program:bootstrap<br/>flake.nix:2190<br/>no user= line -&gt; runs as root<br/>priority=5 autorestart=false one-shot"]
    MGMT["program:management-api<br/>flake.nix:2193<br/>user=devuser bind 0.0.0.0:ENV_MANAGEMENT_API_PORT default 9090<br/>priority=20 REQUIRED_FOR_READINESS=true"]
    SEAL["program:bootstrap-seal<br/>flake.nix:2209<br/>user=devuser priority=99 autorestart=false one-shot<br/>writes /run/agentbox/bootstrap.done, timeout 120s"]
    SOLID["program:solid-pod<br/>flake.nix:2221<br/>gate sovereign_mesh.enabled and local-solid-rs active<br/>user=devuser priority=30 REQUIRED_FOR_READINESS=true"]
    HTTPSB["program:https-bridge<br/>flake.nix:2238<br/>gate sovereign_mesh.enabled and https_bridge<br/>user=devuser priority=32"]
    GATERELAY{"podBridgeEnabled? ADR-2003<br/>flake.nix:1377 relayLocal and pod_bridge"}
    NRELAY1["program:nostr-relay native<br/>flake.nix:2275<br/>bind 127.0.0.1:7777 (AGENTBOX_RELAY_BIND)<br/>user=devuser priority=35 REQUIRED_FOR_READINESS=false"]
    NRELAY2["program:nostr-relay nostr-rs-relay<br/>flake.nix:2287<br/>config /etc/agentbox/nostr-relay.toml<br/>user=devuser priority=35 REQUIRED_FOR_READINESS=false"]
    NGW["program:nostr-gateway<br/>flake.nix:1952<br/>user=devuser priority=234<br/>off switch AGENTBOX_NOSTR_GATEWAY=0"]
    TSD["program:tailscaled<br/>flake.nix:2313<br/>gate networking.tailscale, no user= -&gt; root<br/>socket /var/run/tailscale priority=15"]
    TSU["program:tailscale-up<br/>flake.nix:2334<br/>gate networking.tailscale, no user= -&gt; root<br/>priority=16 autorestart=false one-shot"]
...
```mermaid
flowchart TB
    RAS["program:ruvector-aggregate-sweep<br/>flake.nix:1935<br/>node ruvector-aggregate-sweep.mjs --loop<br/>user=devuser priority=232 startsecs=0"]
    RPD["program:ruvector-pattern-distill<br/>flake.nix:1964<br/>node ruvector-pattern-distill.mjs --loop<br/>user=devuser priority=233 startsecs=0"]
    OCS["program:ontology-condense-scheduler<br/>flake.nix:1982<br/>gate ONTOLOGY_CONDENSE_SCHEDULE and _ENABLED (imageEnv)<br/>user=devuser priority=234 startsecs=0 flock-serialised"]
    PCRON["program:podcast-cron<br/>flake.nix:2493<br/>supercronic over podcast-knowledge-ingest/crontab<br/>user=devuser priority=250"]
    FCRON["program:forum-backup-cron<br/>flake.nix:2513<br/>supercronic over dreamlab-ai-website backup/crontab<br/>user=devuser priority=250, fails loud without CLOUDFLARE_API_TOKEN"]
...
```mermaid
flowchart TB
    QGIS["program:qgis-mcp<br/>flake.nix:1899<br/>gate spatial.qgis, TCP proxy to gui-tools-service:9877<br/>user=devuser priority=230"]
    BLEND["program:blender-mcp<br/>flake.nix:1924<br/>gate spatial.blender, bridges 127.0.0.1:9876 to external GUI sidecar<br/>user=devuser priority=231"]
    JLAB["program:jupyter-lab<br/>flake.nix:1995<br/>gate data_science.jupyter, bind 0.0.0.0:8888 — RESOLVED ADR-2040,<br/>JUPYTER_TOKEN now minted at boot (see AB-07.9), no longer tokenless<br/>user=devuser priority=232"]
    IMGM["program:imagemagick-mcp<br/>flake.nix:2252<br/>gate media.imagemagick<br/>user=devuser priority=210"]
    COMFY["program:comfyui-builtin<br/>flake.nix:2360<br/>gate media.comfyui_builtin, bind 127.0.0.1:8188<br/>user=devuser priority=220"]
    OPF["program:opf-router<br/>flake.nix:2300<br/>gate privacyFilterEnabled (:2298), OPF_PORT default 9092<br/>user=devuser priority=240"]
    DREAM["program:dream-engine<br/>flake.nix:2382<br/>gate dreamEngineEnabled (:1437), LOOM_URL default 192.168.2.132:8084/v1<br/>user=devuser priority=230"]
    CODES["program:code-server<br/>flake.nix:2336<br/>gate toolchains.code_server, bind 0.0.0.0:8080 — RESOLVED ADR-2040,<br/>auth password with a boot-minted credential (see AB-07.9), not auth none<br/>user=devuser priority=50"]
...
flowchart TB
    ...
    HYPR["program:hyprland<br/>flake.nix:2072<br/>user=devuser priority=40"]
    XWAY["program:xwayland-session<br/>flake.nix:2086<br/>user=devuser priority=41"]
    WAYVNC["program:wayvnc<br/>flake.nix:2097<br/>bind 0.0.0.0:5901 user=devuser priority=42"]
    XORG["program:xorg-nvidia<br/>flake.nix:2108<br/>no user= -&gt; root priority=40"]
    I3A["program:i3wm xorg-nvidia branch<br/>flake.nix:2118<br/>user=devuser priority=41"]
    X11VNC["program:x11vnc<br/>flake.nix:2129<br/>bind 0.0.0.0:5901 user=devuser priority=42"]
    XVNC["program:xvnc<br/>flake.nix:2140<br/>bind 0.0.0.0:5901 no user= -&gt; root priority=40"]
    I3B["program:i3wm i3-x11 default branch<br/>flake.nix:2150<br/>user=devuser priority=41"]
    AOE["program:aoe-serve<br/>flake.nix:2410<br/>gate interaction_plane.enabled (:224), bind 127.0.0.1:9095<br/>--auth token --behind-proxy user=devuser priority=45"]
    NIP98["program:nip98-proxy<br/>flake.nix:2432<br/>bind 0.0.0.0:9096, published 9096:9096 in compose<br/>user=devuser priority=46"]
    TAB0["program:tab0-bridge<br/>flake.nix:2462<br/>gate sovereign_mesh.enabled<br/>user=devuser priority=236"]
    TMUX["program:tmux-autostart<br/>flake.nix:2475<br/>user=devuser priority=95 autorestart=false one-shot"]
...
sequenceDiagram
    autonumber
    participant SV as supervisord PID1<br/>flake.nix:2163-2164 supervisord section nodaemon=true
    participant BOOT as program:bootstrap<br/>flake.nix:2190 no user= line means root
    participant MGMT as program:management-api<br/>flake.nix:2193 user=devuser :2196 priority=20 :2200
    participant SOLID as program:solid-pod<br/>flake.nix:2221 user=devuser :2224 priority=30 :2228
    ...
    Note over SV: supervisord itself inherits root from the exec'd<br/>entrypoint-unified.sh Stage A (entrypoint-unified.sh:802)
    SV->>BOOT: spawn priority=5 :2185, environment AGENTBOX_BOOTSTRAP_STAGE=B (:2181)
    Note right of BOOT: no user= line — Stage B (phases 6-8) runs as ROOT,<br/>needed for chown/mkdir under devuser volumes
    SV->>MGMT: spawn priority=20 :2200, user=devuser (:2196)
    SV->>SOLID: spawn priority=30, user=devuser, gated sovereign_mesh.enabled (:2223)
    ...
    SV->>SEAL: spawn priority=99 last, user=devuser (:2209-2215)
...
```mermaid
flowchart TB
    START["program:tmux-autostart<br/>flake.nix:2475, priority=95<br/>runs config/tmux-autostart.sh"]
    W0["window 0 Claude<br/>tmux-autostart.sh:259<br/>tab0-bridge injection target<br/>CLAUDE_CONFIG_DIR=/home/devuser/.claude"]
    W1["window 1 Agent<br/>tmux-autostart.sh:282<br/>agent execution workspace"]
    W2["window 2 Services<br/>tmux-autostart.sh:289<br/>supervisorctl status"]
    W3["window 3 Build<br/>tmux-autostart.sh:295"]
    W4["window 4 Logs<br/>tmux-autostart.sh:303<br/>supervisorctl tail -f management-api, split pane"]
    W5["window 5 System<br/>tmux-autostart.sh:310<br/>systemscape restart loop, split with btm/htop"]
    W6["window 6 VNC<br/>tmux-autostart.sh:327<br/>host shell, display :1 port 5901 status"]
    W7["window 7 Git<br/>tmux-autostart.sh:340<br/>git status in PROJECT dir"]
    W8["window 8 Sessions<br/>tmux-autostart.sh:359<br/>Agent of Empires TUI, presence-detect aoe binary"]
...
sequenceDiagram
    autonumber
    participant E as entrypoint-unified.sh<br/>Phase 4 entrypoint-unified.sh:549-550
    ...
    E->>AM: agentbox-manifest provision-stacks, runs as root Phase 4 (entrypoint-unified.sh:550)
    ...
    Note over E: chown -R 1000:1000 WORKSPACE/profiles after provision-stacks (entrypoint-unified.sh:559)
...
sequenceDiagram
    ...
    participant PID1 as supervisord PID1 env<br/>entrypoint-unified.sh:711 exec
    participant PROG as supervised programs<br/>flake.nix program blocks

    VR->>TOML: _ab_toml_val vault root/pages/format/tui/working/transcripts (:101,126-136)
    alt vault section absent, VAULT_ROOT empty (:102)
        ...
        VR->>VR: export VAULT_ROOT/VAULT_PAGES/VAULT_FORMAT/VAULT_TUI (:139)
        VR->>VR: export ONTOLOGY_PAGES_DIR default VAULT_PAGES, derived (:144-148)
        Note right of VR: DIVERGENCE — vault ENABLED but an explicit<br/>ONTOLOGY_PAGES_DIR differing from VAULT_PAGES<br/>still OVERRIDES it for legacy consumers (:145-146)
    end
    VR->>PID1: exec supervisord inherits VAULT_ROOT/PAGES/FORMAT/TUI/WORKING/TRANSCRIPTS (entrypoint-unified.sh:711)
...
sequenceDiagram
    autonumber
    participant PID1 as supervisord PID1 env<br/>entrypoint-unified.sh:802
    participant RT as runtime-env.sh<br/>entrypoint-unified.sh:2700
    participant BASH as etc profile.d bash<br/>entrypoint-unified.sh:2807
    participant FISH as fish conf.d<br/>entrypoint-unified.sh:2819-2821
    participant TMUX as tmux windows<br/>tmux-autostart.sh:9

    PID1->>RT: Phase 8 writes RUNTIME_ENV_FILE=/run/agentbox/runtime-env.sh (:2700-2769)
    RT->>BASH: ln -sf runtime-env.sh /etc/profile.d/agentbox-runtime.sh (:2807)
    RT->>RT: cp to durable WORKSPACE/.agentbox-runtime-env.sh (:2815)
    RT->>FISH: write conf.d/agentbox-runtime.fish sourcing envfile with fallback to durable copy (:2819-2821)
...
```mermaid
flowchart TB
    COMPOSE["docker-compose.yml:160-162<br/>security_opt no-new-privileges:true<br/>seccomp=./config/seccomp-agentbox.json"]
...
sequenceDiagram
    ...
    participant HUB as program:agentbox-mcp-hub<br/>flake.nix:2539 priority=205 (:2548)
    participant WAIT as wait_for_config<br/>services/agentbox-mcp/src/hub/mod.rs:252
    participant BOOT as program:bootstrap Stage B<br/>config/entrypoint-unified.sh:2281
    participant FS as /run/agentbox/mcp-hub.json

    SV->>HUB: spawn priority=205 while Stage B is still in phase 7, file absent (flake.nix:2548)
    HUB->>WAIT: agentbox-mcp hub --wait-config-secs 120 (flake.nix:2540, default in main.rs:59)
    loop poll every 500 ms (hub/mod.rs:287), log every 15 s (hub/mod.rs:284)
        WAIT->>FS: path.exists
    end
    BOOT->>FS: agentbox-manifest mcp-hub-project writes mcp-hub.json (entrypoint-unified.sh:2281-2284)
    BOOT->>FS: chown 1000:1000 and chmod 600 the projection (entrypoint-unified.sh:2290-2291)
    BOOT->>SV: supervisorctl status agentbox-mcp-hub (entrypoint-unified.sh:2298)
    ...
    alt config present before the timeout
        ...
        Note over SV,HUB: startsecs=130 is LONGER than the wait (flake.nix:2546),<br/>so the exit is a FAILED START, retried startretries=2 (flake.nix:2547)<br/>and then parked FATAL by autorestart=unexpected (flake.nix:2545)
    end
...
**Invariant:** a hub that never receives its projection now fails the start rather than idling, because `startsecs=130` outlives the 120 s wait (`../project/agentbox/flake.nix:2546`, `../project/agentbox/services/agentbox-mcp/src/main.rs:59`).
...
```mermaid
flowchart LR
    SUP["supervisord<br/>flake.nix:2238 [program:https-bridge]<br/>gate sovereign_mesh.enabled and https_bridge"] --> HB["https-proxy.js<br/>self-signed HTTPS to plain-HTTP bridge"]
...
sequenceDiagram
    autonumber
    participant B as program:bootstrap Stage B<br/>config/entrypoint-unified.sh:2608
    ...
    participant RA as reconcile-agents.sh<br/>scripts/reconcile-agents.sh:36
    participant RC as reconcile-commands.sh<br/>scripts/reconcile-commands.sh:22
    participant FS as host-mounted config roots

    B->>RS: CLAUDE_SKILLS_DIR ~/.claude/skills, manifest registered-skills.txt (:2610-2612)
    RS->>FS: symlink each registered name to the baked tree (reconcile-skills.sh:63-67)
    B->>RS: second pass for Codex, ~/.codex/skills with codex-registered-skills.txt (:2619-2624)
    B->>PR: SKILL_ROOT_TARGETS workspace and workspace/project (:2643)
    PR->>FS: retire an unregistered non-overlay dir to .superseded (project-skill-roots.mjs:131)
    B->>RA: AGENT_ROOT_TARGETS plus registered-agents.txt (:2666-2667)
    RA->>FS: retire a vendor-marked or category-filed agent to .superseded (reconcile-agents.sh:150-151)
    RA->>FS: KEPT unmanaged - a flat hand-written agent with no vendor marker is reported, not retired (reconcile-agents.sh:153)
    B->>RC: COMMAND_ROOT_TARGETS three roots, registered-commands.txt (:2682-2683)
    RC->>FS: prune-only sweep, kept names stay, the rest move to .superseded (reconcile-commands.sh:60)
    Note over B,FS: every reconciler is fail-open with a trailing true,<br/>so a broken registry never blocks the boot (:2613, :2668, :2684)
```
...
**Why it is this way.** ADR-2092: the agent roots were ungoverned, so `ruflo init` and `aqe init --auto` accreted 97 agents across two roots, 74 duplicated and 37 byte-divergent, with the nested root shadowing the user root (`../project/agentbox/config/entrypoint-unified.sh:2653-2660`).

**Invariant:** retirement is always to a recoverable `.superseded/` sidecar, never a delete, in all three reconcilers (`../project/agentbox/scripts/reconcile-agents.sh:70`, `../project/agentbox/scripts/reconcile-commands.sh:13`).

**Invariant:** a hand-written agent placed flat in the root with no vendor marker is preserved and reported, so experiments survive the sweep (`../project/agentbox/scripts/reconcile-agents.sh:153`).
