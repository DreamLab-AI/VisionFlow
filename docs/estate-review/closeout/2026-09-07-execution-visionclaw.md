...
| G-24 | Closed: documentation and GUT hosted gates verified | Documentation Quality CI34126476287 and XR workflow34126476293 succeed atfb72df39a. Both blocking XR jobs pass; GUT records83tests/314assertions. Advisory Quest export still fails for absent release keystore and retains test-import diagnostics. This does not certify an APK or headset session. |
...
## Added native visual execution

The user's visual extension is implemented and pushed through `fb72df39a` (visual source `0940bbb4b`, viewport lifecycle fix `6dd349544`). [ADR-2107](../../../../project/docs/adr/ADR-2107-compatible-xr-visual-experience.md) and the [visual report](2026-09-07-xr-visual-upgrade.md) now supersede the earlier observation that no Godot execution was available: isolated Godot binaries ran on the GUI sidecar, with real GL rendering and a rebuilt native extension. The integrated suite passes 83 tests/314 assertions; the Godot 4.6.1 spatial fixture has zero error lines. Two texture-release diagnostics remain at integrated Godot 4.3 shutdown. No headset or Android installation is inferred.

The workflow now imports GUT and runs the pinned Godot 4.3 tests through Xvfb/Mesa GL for meaningful font/layout measurements. Hosted acceptance is tracked separately from the local run. The earlier Quest export reached the signing stage and reported an absent release keystore; it is a concrete operator provisioning problem, not an executed APK. The visual update preserves that target boundary.

Hosted follow-up at `fb72df39a`: Documentation Quality CI run `34126476287` and GL GUT job `101756326729` pass; the latter records 83 tests and 314 assertions. [Hosted receipts](../evidence/execution-2026-09-07/xr-hosted/) qualify G-24 closure. Other XR jobs and the separate signed APK/headset boundary are not inferred green.
