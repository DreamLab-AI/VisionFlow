...
sequenceDiagram
    ...
    GitHubSync->>Pipeline: on_ontology_modified(ontology, correlation_id)
    activate Pipeline

    Pipeline->>Reasoning: TriggerReasoning
    activate Reasoning
    Reasoning->>Reasoning: CustomReasoner.infer_axioms()
    ...
    Pipeline->>Constraint: generate_constraints(axioms)
    activate Constraint
...
sequenceDiagram
    ...
    Pipeline->>GPU: ApplyOntologyConstraints
    GPU->>Circuit: Check circuit state
    Circuit-->>GPU: HALF_OPEN (try again)
    GPU->>GPU: CUDA upload → SUCCESS
    GPU->>Circuit: on_success()
...
sequenceDiagram
    ...
    loop 20 files
        ...
        Pipeline->>ReasoningQ: enqueue_reasoning()
    ...
    loop Process reasoning queue
        ...
        Pipeline->>RateLimiter: try_acquire(1 token)
    ...
    Pipeline->>ConstraintQ: enqueue_constraint()
    ConstraintQ->>GPUQ: enqueue_gpu()
...
sequenceDiagram
    ...
    Pipeline->>Reasoning: TriggerReasoning(ontology_id=1)

    Reasoning->>Cache: get_or_compute(1)
    Cache->>Cache: calculate_checksum(ontology)
    Cache->>DB: SELECT checksum WHERE ontology_id=1

    alt Checksum matches
        DB-->>Cache: Cached checksum matches
        Cache->>DB: SELECT inferred_axioms
        ...
        Reasoning->>Reasoning: CustomReasoner.infer_axioms()
        Reasoning->>DB: INSERT inferred_axioms
    ...
    Pipeline->>Reasoning: InvalidateCache(ontology_id=1)
    Reasoning->>DB: DELETE FROM cache WHERE ontology_id=1
...
sequenceDiagram
    ...
    GitHubSync->>GitHubSync: Generate correlation_id = "xyz-789"

    GitHubSync->>Logger: info("[xyz-789] Starting sync")
    GitHubSync->>Pipeline: on_ontology_modified(correlation_id="xyz-789")

    Pipeline->>Logger: info("[xyz-789] Pipeline triggered")
    Pipeline->>Reasoning: TriggerReasoning(correlation_id="xyz-789")
    ...
    Pipeline->>GPU: ApplyOntologyConstraints(correlation_id="xyz-789")
    ...
    Pipeline->>Metrics: record("xyz-789", total_duration=150ms)
...
sequenceDiagram
    ...
    Pipeline->>Reasoning: TriggerReasoning
    Reasoning->>Reasoning: infer_axioms()
    Reasoning-->>Pipeline: ERROR (timeout)

    Pipeline->>Retry: with_retry(f, max=3, backoff=100ms)

    Note over Retry: Attempt 1/3
    Retry->>Reasoning: infer_axioms()
...
sequenceDiagram
    ...
    API->>Pipeline: on_ontology_modified()
    Pipeline-->>API: Stats
    API-->>Admin: {"status": "triggered", "correlation_id": "abc-123"}

    Admin->>API: GET /api/admin/pipeline/status
    API->>Pipeline: get_status()
    API->>EventBus: get_stats()
    API-->>Admin: {"status": "running", "queue_sizes": {...}}
    ...
    API->>EventBus: get_events_by_correlation("abc-123")
