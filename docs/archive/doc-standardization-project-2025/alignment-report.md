...
1. **GPU Physics & Semantic Analysis**
   ...
   - **Files**: `gpu-manager-actor.rs`, `force-compute-actor.rs`, `stress-majorization-actor.rs`, `clustering-actor.rs`, `anomaly-detection-actor.rs`, `ontology-constraint-actor.rs`
...
2. **Ontology & Reasoning Pipeline**
   ...
   - **Actual Code**: `/src/ontology/` (6 modules), `/src/reasoning/` (5 modules), `/src/services/ontology-pipeline-service.rs`
...
3. **Neo4j Integration**
   ...
   - **Actual Code**: `/src/adapters/neo4j-adapter.rs`, `/src/adapters/neo4j-settings-repository.rs` (implemented and active)
...
9. **Application State & Configuration**
   ...
   - **Actual Code**: `/src/app-state.rs` (28,931 bytes), `/src/config/` (module)
...
10. **Event System**
    ...
    - **Actual Code**: `/src/events/` (7 files: bus, store, types, handlers, middleware, inference-triggers)
...
11. **GitHub Sync Service**
    ...
    - **Actual Code**: `/src/services/github-sync-service.rs`, `/src/services/github/` (3 modules)
    - **Status**: 🟢 Differential sync, SHA1 hashing, FORCE-FULL-SYNC documented
...
12. **WebSocket Protocols**
    ...
    - **Actual Code**: `/src/handlers/*-websocket-handler.rs` (5 websocket handlers), `/src/protocols/binary-settings-protocol.rs`
...
13. **Settings Management** ✅ **MIGRATION COMPLETE (November 2025)**
    ...
    - **Actual Code**: Neo4j settings repository (`/src/adapters/neo4j-settings-repository.rs` - 25,745 bytes, actively used in `main.rs`)
    ...
    - **Evidence**: `main.rs:162` shows `Neo4jSettingsRepository::new(settings-config).await`
...
14. **Adapters Layer**
    ...
    - **Gap**: Missing documentation for these adapters:
      - `actix-physics-adapter.rs`
      - `actix-semantic-adapter.rs`
      - `actor-graph-repository.rs`
      - `gpu-semantic-analyzer.rs`
      - `physics-orchestrator-adapter.rs`
      - `whelk-inference-engine.rs`

15. **Services Layer**
    - **Documentation**: Scattered across multiple guides
    - **Actual Code**: `/src/services/` (10+ service modules including ontology-pipeline, github-sync, speech, nostr, ragflow, settings-watcher, settings-broadcast)
...
17. **Telemetry & Logging**
    ...
    - **Actual Code**: `/src/telemetry/` (3 files: agent-telemetry, test-logging), `/src/utils/advanced-logging.rs`
...
20. **Models & Types**
    ...
    - **Actual Code**: `/src/models/` (user-settings, protected-settings), `/src/types/` (module)
...
21. **Constraints System**
    ...
    - **Actual Code**: `/src/constraints/` (module), `/src/handlers/constraints-handler.rs`, `/src/handlers/api-handler/constraints/`
...
22. **Performance & Benchmarking**
    ...
    - **Actual Code**: `/src/performance/` (module including settings-benchmark.rs)
...
23. **Physics System**
    ...
    - **Actual Code**: `/src/physics/` (module), `/src/handlers/physics-handler.rs`, GPU actors
...
24. **Protocols**
    ...
    - **Actual Code**: `/src/protocols/` (binary-settings-protocol.rs)
...
25. **Binary Utilities** (`/src/bin/`)
    - **Files**: `migrate-settings-to-neo4j.rs` (migration utility)
...
26. **Test Infrastructure** (`/src/handlers/tests/`, `/src/tests/`)
    - **Files**: `settings-tests.rs`, various test modules
...
2. **GraphServiceActor Deprecation** (38 occurrences across 8 files) ⚠️ **ARCHITECTURE CHANGE**
   ...
   - **Files Referencing GraphServiceActor**:
     ...
     - `/docs/concepts/architecture/quick-reference.md` (2 references)
...
3. **Three-Database Architecture**
   ...
   - **Recommended Action**: Search for "knowledge-graph.db", "ontology.db", "settings.db" references and update to unified.db
...
| 3 | app-state | `/src/app-state.rs` | 1 | configuration.md | 🟢 Good |
