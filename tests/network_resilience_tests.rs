                ...
                Err(error) => {
                    ...
                    if attempts <= self.max_retries {
                        let delay = Duration::from_millis(
                             ...
                             * self.backoff_multiplier.powi(attempts as i32 - 1)) as u64
                        );
...
impl NetworkResilienceTestSuite {
    ...
    async fn test_retry_policy_basic(&mut self) {
        ...
        let result = retry_policy.execute_with_retry(|| {
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    mock_service.make_request(1).await
                })
            })
        }).await;
        ...
        match result {
            Ok(response) => {
                ...
                if mock_service.get_call_count() != 1 {
                    eprintln!("Should make exactly 1 call for successful operation, got {}", 
        ...
        let result = retry_policy.execute_with_retry(|| {
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    mock_service.make_request(2).await
                })
            })
        }).await;
        ...
        let result = retry_policy.execute_with_retry(|| {
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    mock_service.make_request(3).await
                })
            })
        }).await;
        ...
        if final_call_count != 4 { // initial + 3 retries
            eprintln!("Should make exactly 4 calls (1 + 3 retries), got {}", final_call_count);
            all_passed = false;
        }
    ...
    async fn test_fixed_delay_retry(&mut self) {
        ...
        if call_count > 3 {
            eprintln!("Should not exceed max retries + 1, got {} calls", call_count);
            all_passed = false;
        }
    ...
    async fn test_network_error_classifications(&mut self) {
        ...
        for (i, error) in errors.iter().enumerate() {
            ...
            match error {
                ...
                NetworkError::HTTPError { url, status: Some(status), .. } => {
            ...
            let should_retry = match error {
                ...
                NetworkError::HTTPError { status: Some(503), .. } => true, // Service unavailable
                NetworkError::HTTPError { status: Some(429), .. } => true, // Rate limited
                NetworkError::HTTPError { status: Some(404), .. } => false, // Not found
                NetworkError::HTTPError { status: Some(401), .. } => false, // Unauthorized
    ...
    async fn test_connection_failures(&mut self) {
        ...
        match result {
            Err(NetworkError::Timeout { operation, timeout_ms }) => {
        ...
        match result {
            Err(NetworkError::HTTPError { status: Some(503), .. }) => {
                // Expected
            },
            _ => {
    ...
    async fn test_service_degradation(&mut self) {
        ...
        for (failure_rate, scenario) in degradation_scenarios {
            ...
            if actual_failure_rate < expected_min || actual_failure_rate > expected_max {
                eprintln!("Scenario '{}': expected failure rate {:.1}, got {:.1}", 
                         scenario, failure_rate, actual_failure_rate);
                // This might be flaky due to pseudo-randomness, so we'll log but not fail
            }
            
            println!("Scenario '{}': {}/{} requests failed ({:.1}% failure rate)", 
                    scenario, failure_count, total_requests, actual_failure_rate * 100.0);
        }
        ...
        match result {
            ...
            Err(other_error) => {
                eprintln!("Unexpected error type during service degradation: {:?}", other_error);
                all_passed = false;
            },
            Ok(_) => {
    ...
    async fn test_concurrent_requests_with_failures(&mut self) {
        ...
        for i in 0..concurrent_requests {
            ...
            let handle = tokio::spawn(async move {
                service_clone.make_request(i).await
            });
            handles.push(handle);
        }
        ...
        if total_calls != concurrent_requests as usize {
            eprintln!("Should make exactly {} calls, got {}", concurrent_requests, total_calls);
            all_passed = false;
        }
        
        if success_count + failure_count != concurrent_requests {
            eprintln!("Success + failure count should equal total requests: {} + {} != {}", 
                     success_count, failure_count, concurrent_requests);
            all_passed = false;
        }
        
        let actual_failure_rate = failure_count as f64 / concurrent_requests as f64;
        println!("Concurrent requests: {}/{} failed ({:.1}% failure rate)", 
                failure_count, concurrent_requests, actual_failure_rate * 100.0);
    ...
    async fn test_retry_with_circuit_breaker(&mut self) {
        ...
        let execute_with_circuit_breaker = |service: Arc<MockNetworkService>, 
        ...
        for i in 0..5 {
            ...
            let _ = retry_policy.execute_with_retry(|| {
    ...
    async fn test_rate_limiting_behavior(&mut self) {
        ...
        for (i, &time) in request_times.iter().enumerate() {
            let expected_min_time = Duration::from_millis((i as u64) * rate_limit_delay.as_millis() as u64);
            
            if time < expected_min_time && i > 0 {
                eprintln!("Request {} completed too quickly: {:?} < {:?}", 
    ...
    async fn test_health_check_integration(&mut self) {
        ...
        if status_after_some_failures != HealthStatus::Degraded {
            eprintln!("Health status should be degraded after some failures, got {:?}", status_after_some_failures);
            all_passed = false;
        }
        ...
        if status_after_many_failures != HealthStatus::Unhealthy {
            eprintln!("Health status should be unhealthy after many failures, got {:?}", status_after_many_failures);
            all_passed = false;
        }
        ...
        if status_after_one_success != HealthStatus::Degraded {
            eprintln!("Health status should be degraded after one success from unhealthy, got {:?}", status_after_one_success);
            all_passed = false;
        }
        ...
        if final_status != HealthStatus::Healthy {
            eprintln!("Health status should be healthy after sustained success, got {:?}", final_status);
            all_passed = false;
        }
...
async fn run_network_resilience_validation() {
    ...
    assert!(test_suite.failed_tests == 0, "All network resilience tests should pass");
