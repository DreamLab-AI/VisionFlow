...
mod ptx_comprehensive_tests {
    ...
    fn test_kernel_symbol_completeness() {
        ...
        println!(
            "  Found: {}/{}",
            found_kernels.len(),
            found_kernels.len() + missing_kernels.len()
        );
        ...
        for kernel in phase1_kernels {
            assert!(
                found_kernels.contains(&kernel),
                "Phase 1 kernel '{}' must be present in PTX",
                kernel
            );
        }
    }
    ...
    fn test_cold_start_performance() {
        ...
        assert!(
            module_time.as_millis() < 1000,
            "Module creation should be <1s"
        );
    ...
    fn test_ptx_content_validation() {
        ...
        assert!(
            ptx.contains(".target"),
            "PTX should contain .target directive"
        );
        assert!(
            ptx.contains(".address_size"),
            "PTX should contain .address_size directive"
        );

        // Check for required sections
        assert!(
            ptx.contains(".visible .entry"),
            "PTX should contain kernel entry points"
        );
        ...
        assert!(
            found_patterns >= 3,
            "Should find at least 3 kernel patterns in PTX, found {}",
            found_patterns
        );
        ...
        assert!(
            ptx.len() > 1000,
            "PTX should be substantial (>1KB), got {} bytes",
            ptx.len()
        );
        assert!(
            ptx.len() < 10_000_000,
            "PTX should not be excessive (<10MB), got {} bytes",
            ptx.len()
        );
    ...
    fn test_cuda_arch_detection() {
        ...
        assert!(
            detected_arch >= 50,
            "CUDA architecture should be >= 50 (Maxwell)"
        );
        assert!(
            detected_arch <= 90,
            "CUDA architecture should be <= 90 (future proof)"
        );
        ...
        if is_common {
            ...
            println!(
                "  ⚠️ Detected architecture {} may need special handling",
                detected_arch
            );
        }
