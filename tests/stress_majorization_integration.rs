...
fn check_position_explosion(nodes: &[BinaryNodeData], threshold: f32) -> bool {
    for node in nodes {
        let distance_from_origin = (node.position.x * node.position.x + 
                                  ...
                                  node.position.z * node.position.z).sqrt();
...
fn check_numerical_stability(nodes: &[BinaryNodeData]) -> bool {
    for node in nodes {
        if !node.position.x.is_finite() || !node.position.y.is_finite() || !node.position.z.is_finite() ||
           !node.velocity.x.is_finite() || !node.velocity.y.is_finite() || !node.velocity.z.is_finite() {
            return false;
        }
    }
    true
...
async fn run_single_test(
    ...
    let max_displacement = if initial_nodes.len() == final_nodes.len() {
        initial_nodes.iter().zip(final_nodes.iter())
    ...
    Ok(TestRunResult {
        ...
        energy_conserved,
        position_explosion: check_position_explosion(&final_nodes, config.max_displacement_threshold * 2.0),
        numerical_stability: check_numerical_stability(&final_nodes),
    })
}

/// Run comprehensive stability tests
async fn run_stability_tests(config: StabilityTestConfig) -> Result<StabilityTestResults, Box<dyn std::error::Error>> {
    ...
    let params_msg = UpdateAdvancedParams { params: advanced_params };
    ...
    for run_id in 0..config.runs {
        ...
        match run_single_test(run_id, &config, &gpu_actor).await {
            Ok(result) => {
                println!("  Run {} - Converged: {}, Stress: {:.2}, Displacement: {:.2}, Stable: {}",
                         result.run_id, result.converged, result.final_stress, 
                         result.max_displacement, result.numerical_stability);
                results.push(result);
            }
            Err(e) => {
    ...
    let stable_count = results.iter().filter(|r| r.numerical_stability && !r.position_explosion).count() as f32;
    ...
    let valid_results: Vec<_> = results.iter().filter(|r| r.final_stress.is_finite()).collect();
    ...
    let avg_displacement = if !valid_results.is_empty() {
        valid_results.iter().map(|r| r.max_displacement).sum::<f32>() / valid_results.len() as f32
    } else {
        0.0
    };
    
    let avg_computation_time = if !valid_results.is_empty() {
        valid_results.iter().map(|r| r.computation_time_ms).sum::<u64>() / valid_results.len() as u64
    } else {
        0
    };
...
async fn test_stress_majorization_stability_small_graph() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Energy Conservation Rate: {:.1}%", results.energy_conservation_rate * 100.0);
    println!("Average Stress: {:.2}", results.avg_stress);
    println!("Average Displacement: {:.2}", results.avg_displacement);
    println!("Average Computation Time: {}ms", results.avg_computation_time);
    println!("Overall Result: {}", if results.passed { "PASS" } else { "FAIL" });
...
async fn test_stress_majorization_safety_controls() -> Result<(), Box<dyn std::error::Error>> {
    ...
    gpu_actor.send(UpdateAdvancedParams { params: dangerous_params }).await??;
    ...
    gpu_actor.send(UpdateGPUGraphData { graph_data: problematic_graph }).await??;
    gpu_actor.send(UpdateConstraints { constraints: conflicting_constraints }).await??;
    ...
    assert!(stats.success_rate > 0.0 || stats.is_emergency_stopped || stats.consecutive_failures >= 3,
...
async fn test_stress_majorization_parameter_updates() -> Result<(), Box<dyn std::error::Error>> {
    ...
    gpu_actor.send(InitializeGPU { max_nodes: 50, max_edges: 100 }).await??;
    ...
    gpu_actor.send(UpdateStressMajorizationParams { params: params1.clone() }).await??;
    ...
    gpu_actor.send(UpdateStressMajorizationParams { params: params2.clone() }).await??;
