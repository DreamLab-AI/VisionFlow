...
mod settings_sync_tests {
    ...
    async fn test_get_settings_endpoint() {
        ...
        let glow = body["visualisation"]["glow"].as_object()
    ...
    async fn test_update_bloom_settings_invalid() {
        ...
        for (invalid_setting, description) in test_data::invalid_bloom_settings() {
            ...
            assert!(
                ...
                "Failed validation test for: {}", description
            );
            
            let body: Value = test::read_body_json(resp).await;
            assert!(body.get("error").is_some(), "Error message should be present for: {}", description);
        }
    }
    ...
    async fn test_rate_limiting() {
        ...
        for i in 0..15 {  // Exceed typical rate limits
            let req = test::TestRequest::post()
                .uri("/settings")
    ...
    async fn test_validation_stats_endpoint() {
        ...
        let security_features = body["security_features"].as_array()
            .expect("Security features should be an array");
            
        assert!(security_features.iter().any(|f| f.as_str() == Some("comprehensive_input_validation")));
        assert!(security_features.iter().any(|f| f.as_str() == Some("rate_limiting")));
    }
}
...
mod error_handling_tests {
    ...
    async fn test_concurrent_settings_updates() {
        ...
        for status in results {
            assert!(!status.is_server_error(), "Server error during concurrent updates");
        }
    }
}
...
mod performance_tests {
    ...
    async fn test_settings_response_time() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        assert!(duration.as_millis() < 100, "Settings fetch should be fast: {:?}", duration);
    }
    ...
    async fn test_large_settings_update() {
        ...
        for i in 0..100 {
            large_update["visualisation"]["graphs"]["logseq"]["physics"][format!("customParam{}", i)] = json!(i as f64 * 0.01);
        }
