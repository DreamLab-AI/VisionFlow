...
async fn test_settings_json_serialization() {
    ...
    assert!(rendering.get("ambientLightIntensity").is_some(), "ambientLightIntensity should be in camelCase");
    assert!(rendering.get("directionalLightIntensity").is_some(), "directionalLightIntensity should be in camelCase");
    assert!(rendering.get("enableAmbientOcclusion").is_some(), "enableAmbientOcclusion should be in camelCase");
    assert!(rendering.get("enableAntialiasing").is_some(), "enableAntialiasing should be in camelCase");
    
    // These should NOT exist in snake_case
    assert!(rendering.get("ambient_light_intensity").is_none(), "ambient_light_intensity should not exist (should be camelCase)");
    assert!(rendering.get("directional_light_intensity").is_none(), "directional_light_intensity should not exist (should be camelCase)");
...
async fn test_json_path_access() {
    ...
    let ambient_light = settings.get_json_by_path("visualisation.rendering.ambientLightIntensity")
        .expect("Failed to get ambientLightIntensity");
    
    let enable_ao = settings.get_json_by_path("visualisation.rendering.enableAmbientOcclusion")
        .expect("Failed to get enableAmbientOcclusion");
    
    // Verify values are correct types
    assert!(ambient_light.is_f64(), "ambientLightIntensity should be a number");
    assert!(enable_ao.is_boolean(), "enableAmbientOcclusion should be a boolean");
...
async fn test_json_path_set_and_get() {
    ...
    settings.set_json_by_path("visualisation.rendering.ambientLightIntensity", new_intensity.clone())
        .expect("Failed to set ambientLightIntensity");
    
    // Get the value back
    let retrieved_value = settings.get_json_by_path("visualisation.rendering.ambientLightIntensity")
        .expect("Failed to get ambientLightIntensity");
    
    assert_eq!(retrieved_value, new_intensity, "Set and retrieved values should match");
    
    // Verify it's also reflected in the actual struct field
    assert_eq!(settings.visualisation.rendering.ambient_light_intensity, 0.8, "Struct field should be updated");
...
async fn test_physics_parameter_sync() {
    ...
    for (path, value) in physics_updates {
        settings.set_json_by_path(path, value.clone())
            .expect(&format!("Failed to set {}", path));
        
        // Verify we can get it back
        let retrieved = settings.get_json_by_path(path)
...
async fn test_batch_settings_update() {
    ...
    let batch_updates = vec![
        ...
        ("visualisation.rendering.directionalLightIntensity", json!(1.2)),
        ("visualisation.rendering.enableAmbientOcclusion", json!(false)),
    ...
    for (path, expected_value) in &batch_updates {
        let actual_value = settings.get_json_by_path(path)
            .expect(&format!("Failed to get {}", path));
        assert_eq!(&actual_value, expected_value, "Batch update failed for {}", path);
    }
    
    // Verify struct fields are updated
    assert_eq!(settings.visualisation.rendering.ambient_light_intensity, 0.7);
    assert_eq!(settings.visualisation.rendering.directional_light_intensity, 1.2);
    assert_eq!(settings.visualisation.rendering.enable_ambient_occlusion, false);
...
fn test_client_server_serialization_compatibility() {
    ...
    assert!(rendering.get("ambientLightIntensity").is_some(), "Should have camelCase ambientLightIntensity");
    assert!(rendering.get("enableAmbientOcclusion").is_some(), "Should have camelCase enableAmbientOcclusion");
    assert!(rendering.get("ambient_light_intensity").is_none(), "Should NOT have snake_case ambient_light_intensity");
    assert!(rendering.get("enable_ambient_occlusion").is_none(), "Should NOT have snake_case enable_ambient_occlusion");
    
    // Verify the serialized values match what we set (with some tolerance for floats)
    assert!((rendering.get("ambientLightIntensity").unwrap().as_f64().unwrap() - 0.6).abs() < 0.01);
    assert!((rendering.get("directionalLightIntensity").unwrap().as_f64().unwrap() - 1.1).abs() < 0.01);
    assert_eq!(rendering.get("enableAmbientOcclusion").unwrap().as_bool().unwrap(), true);
    assert_eq!(rendering.get("enableAntialiasing").unwrap().as_bool().unwrap(), false);
