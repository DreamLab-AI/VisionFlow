...
mod batch_update_integration_tests {
    ...
    async fn test_batch_update_endpoint_with_camel_case() {
        ...
        assert!(resp.status().is_success(), "Batch update failed with status: {}", resp.status());

        let body: serde_json::Value = test::read_body_json(resp).await;
        
        // Verify response structure
        assert_eq!(body["success"], true);
        assert!(body["message"].as_str().unwrap().contains("Successfully processed"));
        ...
        for value in values {
            assert_eq!(value["success"], true, "Update failed for path: {}", value["path"]);
            assert!(value["value"].is_object() || value["value"].is_bool() || value["value"].is_string() || value["value"].is_number());
        }
    ...
    async fn test_batch_update_with_invalid_paths() {
        ...
        assert!(values[1]["error"].as_str().unwrap().contains("does not exist"));
