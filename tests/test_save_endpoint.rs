...
use webxr::{
    ...
    actors::{SettingsActor, messages::GetSettings},
};
use actix::Actor;

// Helper function to create a test app with settings
async fn create_test_app_with_settings(settings: AppFullSettings) -> (
    actix_web::test::TestServer,
    actix::Addr<SettingsActor>,
) {
    let settings_addr = SettingsActor::new(
...
async fn test_save_without_persistence() {
    ...
    assert!(body["error"].as_str().unwrap().contains("Settings persistence is disabled"));
}
...
async fn test_save_with_updates() {
    ...
    assert_eq!(current_settings.visualisation.rendering.ambient_light_intensity, 2.5);
    assert_eq!(current_settings.visualisation.rendering.directional_light_intensity, 3.0);
