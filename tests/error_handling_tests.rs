...
impl ErrorHandlingTestSuite {
    ...
    async fn test_vision_flow_error_types(&mut self) {
        ...
        let errors = vec![
            ...
            VisionFlowError::GPU(GPUError::DeviceInitializationFailed("CUDA not found".to_string())),
            ...
            VisionFlowError::IO(std::io::Error::new(std::io::ErrorKind::NotFound, "File missing")),
    ...
    async fn test_error_conversion(&mut self) {
        ...
        match vision_error {
            VisionFlowError::Actor(ActorError::RuntimeFailure { ref actor_name, ref reason }) => {
        ...
        match vision_error {
            VisionFlowError::GPU(GPUError::MemoryAllocationFailed { requested_bytes, ref reason }) => {
    ...
    async fn test_error_context_preservation(&mut self) {
        ...
        let with_context = io_result.with_context(|| "Failed to load application configuration".to_string());
        
        match with_context {
            Err(VisionFlowError::Generic { ref message, ref source }) => {
        ...
        match with_actor_context {
            Err(VisionFlowError::Actor(ActorError::RuntimeFailure { ref actor_name, ref reason })) => {
        ...
        match with_gpu_context {
            Err(VisionFlowError::GPU(GPUError::KernelExecutionFailed { ref kernel_name, ref reason })) => {
    ...
    async fn test_error_propagation_chains(&mut self) {
        ...
        fn layer_3_function() -> Result<String, std::io::Error> {
            Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Database file not found"))
        }
            ...
            Err(VisionFlowError::Generic { ref message, ref source }) => {
                ...
                if let Some(source) = source {
                    ...
                    if !source_str.contains("Failed to read user data") {
                        eprintln!("Intermediate error context not preserved in source chain: {}", source_str);
                        all_passed = false;
                    }
                } else {
        ...
        for actor_error in actor_errors {
            ...
            match &actor_error {
                ...
                ActorError::SupervisionFailed { supervisor, supervised, reason } => {
                    if !display_str.contains(supervisor) || !display_str.contains(supervised) || !display_str.contains(reason) {
                        eprintln!("SupervisionFailed error missing supervisor, supervised, or reason");
                        all_passed = false;
                    }
                },
        ...
        for gpu_error in gpu_errors {
            ...
            match &gpu_error {
                ...
                GPUError::MemoryAllocationFailed { requested_bytes, reason } => {
                    if !display_str.contains(&requested_bytes.to_string()) || !display_str.contains(reason) {
                        eprintln!("MemoryAllocationFailed missing bytes or reason: {}", display_str);
                        all_passed = false;
                    }
                },
                GPUError::KernelExecutionFailed { kernel_name, reason } => {
                    if !display_str.contains(kernel_name) || !display_str.contains(reason) {
                        eprintln!("KernelExecutionFailed missing kernel name or reason: {}", display_str);
                        all_passed = false;
                    }
                },
                GPUError::DataTransferFailed { direction, reason } => {
                    ...
                    if !display_str.contains(&direction_str) || !display_str.contains(reason) {
                        eprintln!("DataTransferFailed missing direction or reason: {}", display_str);
                        all_passed = false;
                    }
                },
    ...
    async fn test_settings_error_scenarios(&mut self) {
        ...
        let settings_errors = vec![
            ...
            SettingsError::CacheError("Cache corrupted, unable to read cached settings".to_string()),
        ];
        ...
        for settings_error in settings_errors {
            ...
            if display_str.is_empty() {
                eprintln!("SettingsError has empty display string: {:?}", settings_error);
                all_passed = false;
                continue;
            }
            ...
            match &settings_error {
                ...
                SettingsError::ValidationFailed { setting_path, reason } => {
                    if !display_str.contains(setting_path) || !display_str.contains(reason) {
                        eprintln!("ValidationFailed missing setting path or reason: {}", display_str);
                        all_passed = false;
                    }
                },
        ...
        for network_error in network_errors {
            ...
            match &network_error {
                NetworkError::ConnectionFailed { host, port, reason } => {
                    if !display_str.contains(host) || !display_str.contains(&port.to_string()) || !display_str.contains(reason) {
                        eprintln!("ConnectionFailed missing host, port, or reason: {}", display_str);
                        all_passed = false;
                    }
                },
                ...
                NetworkError::Timeout { operation, timeout_ms } => {
                    if !display_str.contains(operation) || !display_str.contains(&timeout_ms.to_string()) {
    ...
    async fn test_error_recovery_patterns(&mut self) {
        ...
        for i in 0..safety_config.cpu_fallback_threshold {
            ...
            if should_fallback != expected_fallback {
                eprintln!("Fallback threshold logic incorrect at failure {}: expected {}, got {}", 
    ...
    async fn test_error_logging_integration(&mut self) {
        ...
        let errors = vec![
            ...
            VisionFlowError::GPU(GPUError::DeviceInitializationFailed("No CUDA devices".to_string())),
            VisionFlowError::Network(NetworkError::ConnectionFailed {
                host: "localhost".to_string(),
                port: 8080,
        ...
        for (i, error) in errors.iter().enumerate() {
            ...
            match error {
                VisionFlowError::Actor(actor_err) => {
                    match actor_err {
                        ActorError::StartupFailed { actor_name, reason } => {
                ...
                VisionFlowError::Network(net_err) => {
                    match net_err {
                        NetworkError::ConnectionFailed { host, port, reason } => {
    ...
    async fn test_error_serialization(&mut self) {
        ...
        for (i, error) in errors.iter().enumerate() {
            ...
            match error {
                ...
                VisionFlowError::GPU(GPUError::KernelExecutionFailed { kernel_name, reason }) => {
                ...
                VisionFlowError::Settings(SettingsError::ValidationFailed { setting_path, reason }) => {
                    if !serialized.contains(setting_path) || !serialized.contains(reason) {
                        eprintln!("Settings error serialization missing key info: {}", serialized);
                        all_passed = false;
                    }
                },
                VisionFlowError::Network(NetworkError::Timeout { operation, timeout_ms }) => {
                    if !serialized.contains(operation) || !serialized.contains(&timeout_ms.to_string()) {
                        eprintln!("Network error serialization missing key info: {}", serialized);
                        all_passed = false;
                    }
                },
                _ => {}
            ...
            if debug_serialized.len() <= serialized.len() {
                eprintln!("Error {} debug format should be more detailed than display format", i);
                all_passed = false;
            }
        }
...
async fn run_error_handling_validation() {
    ...
    assert!(test_suite.failed_tests == 0, "All error handling tests should pass");
    assert!(test_suite.passed_tests > 10, "Should have meaningful test coverage");
}