...
mod bloom_glow_validation_tests {
    ...
    fn test_bloom_specific_strength_fields() {
        ...
        assert!(
            result.is_ok(),
            "Should accept valid bloom strength fields: {:?}",
            result.err()
        );
        ...
        assert!(result
            .unwrap_err()
            .contains("edgeBloomStrength must be between 0.0 and 1.0"));
    }
    ...
    fn test_rendering_settings_validation() {
        ...
        assert!(
            result.is_ok(),
            "Should accept rendering with bloom field: {:?}",
            result.err()
        );
        ...
        assert!(
            result.is_ok(),
            "Should accept rendering with glow field: {:?}",
            result.err()
        );
    }
    ...
    fn test_field_mapping_in_merge_process() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully merge bloom update: {:?}",
            result.err()
        );
        ...
        assert!(
            glow_field.is_some(),
            "Should find glow field after bloom update"
        );

        // When converting to camelCase for client, it should be 'bloom'
        let camel_case = settings
            .to_camel_case_json()
            .expect("Should convert to camelCase");
        ...
        assert!(
            bloom_field.is_some(),
            "Should find bloom field in client JSON"
        );
    }
    ...
    fn test_comprehensive_validation_service() {
        ...
        assert!(
            result.is_ok(),
            "Should validate complex payload with bloom effects: {:?}",
            result.err()
        );

        if let Ok(sanitized) = result {
            // Ensure the bloom field is preserved in the sanitized output
            let bloom_field = sanitized.pointer("/visualisation/rendering/bloom");
            assert!(
                bloom_field.is_some(),
                "Bloom field should be preserved in sanitized output"
            );
        }
    }
}
