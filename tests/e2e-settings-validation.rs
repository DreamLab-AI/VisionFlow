...
mod api_endpoint_tests {
    ...
    async fn test_get_settings_structure() {
        ...
        assert!(
            body.get("visualisation").is_some(),
            "Missing visualisation section"
        );
        ...
        assert!(
            glow.get("nodeGlowStrength").is_some(),
            "Missing glow.nodeGlowStrength"
        );
        assert!(
            glow.get("edgeGlowStrength").is_some(),
            "Missing glow.edgeGlowStrength"
        );
        ...
        for test_case in validation_test_data::bloom_field_tests() {
            ...
            if test_case.should_pass {
                ...
                if let Some(expected_pattern) = test_case.expected_error_pattern {
                    let body: Value = test::read_body_json(response).await;
                    let error_message = body.get("error").and_then(|e| e.as_str()).unwrap_or("");
    ...
    async fn test_settings_reset_robustness() {
        ...
        assert_ne!(
            base_color, "#ff0000",
            "Color should have been reset from #ff0000"
        );
    }
    ...
    async fn test_validation_stats_endpoint() {
        ...
        assert!(
            body.get("validation_service").is_some(),
            "Missing validation_service"
        );
        assert!(body.get("rate_limiting").is_some(), "Missing rate_limiting");
        assert!(
            body.get("security_features").is_some(),
            "Missing security_features"
        );
        assert!(
            body.get("endpoints_protected").is_some(),
            "Missing endpoints_protected"
        );

        let security_features = body
...
mod edge_case_tests {
    ...
    async fn test_auto_balance_sync_robustness() {
        ...
        let logseq_auto_balance = body["visualisation"]["graphs"]["logseq"]["physics"]
            ["autoBalance"]
            .as_bool()
            .expect("logseq should have autoBalance");

        let visionflow_auto_balance = body["visualisation"]["graphs"]["visionflow"]["physics"]
            ["autoBalance"]
            .as_bool()
        ...
        assert!(
            visionflow_auto_balance,
            "visionflow autoBalance should be true (synced)"
        );
    }
}
...
mod integration_tests {
    ...
    async fn test_physics_endpoint_integration() {
        ...
        let response = server
            .post_json("/api/physics/update", physics_update)
            .await;
        assert_eq!(response.status(), StatusCode::OK);
        ...
        let visionflow_spring_k = body["visualisation"]["graphs"]["visionflow"]["physics"]
            ["springK"]
            .as_f64()
    ...
    async fn test_clustering_endpoint_integration() {
        ...
        let response = server
            .post_json("/api/clustering/algorithm", clustering_update)
            .await;
        assert_eq!(response.status(), StatusCode::OK);
    ...
    async fn test_stress_optimization_endpoint() {
        ...
        let response = server
            .post_json("/api/stress/optimization", stress_update)
            .await;
        assert_eq!(response.status(), StatusCode::OK);
