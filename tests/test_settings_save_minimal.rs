...
fn test_settings_save_to_file() {
    ...
    assert!(
        saved_content.contains("intensity: 2.5"),
        "Intensity not found in saved file"
    );
    assert!(
        saved_content.contains("radius: 0.75"),
        "Radius not found in saved file"
    );
    assert!(
        saved_content.contains("ambient_light_intensity: 3"),
        "Ambient light intensity not found in saved file"
    );
...
fn test_settings_save_disabled() {
    ...
    assert!(
        !Path::new(&settings_path).exists(),
        "Settings file should not be created when persist_settings is false"
    );
...
fn test_settings_merge_and_save() {
    ...
    let saved_content = fs::read_to_string(&settings_path).unwrap();
    assert!(
        saved_content.contains("intensity: 3.5"),
        "Merged intensity not found"
    );
    assert!(
        saved_content.contains("threshold: 0.9"),
        "Merged threshold not found"
    );
    assert!(
        saved_content.contains("directional_light_intensity: 2"),
        "Merged directional light not found"
    );
