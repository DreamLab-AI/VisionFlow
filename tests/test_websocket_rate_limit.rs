use webxr::utils::validation::rate_limit::{RateLimitConfig, EndpointRateLimits};
...
fn test_socket_flow_updates_rate_limit() {
    ...
    assert_eq!(config.requests_per_minute, 300, "Should allow 300 requests per minute for 5Hz updates");
    ...
    assert_eq!(config.max_violations, 10, "Should be more lenient with violations for real-time data");
...
fn test_rate_limit_calculation() {
    ...
    assert_eq!(min_interval_ms, 200, "Minimum interval should be 200ms for 5Hz updates");
    ...
    assert!(updates_per_minute <= requests_per_minute, "5Hz update rate should fit within rate limit");
}