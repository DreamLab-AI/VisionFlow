...
fn test_values_compatible_with_conversion() {
    ...
    assert!(values_compatible(&existing, &new_string), "String should be compatible with number");
    assert!(values_compatible(&existing, &new_number), "Number should be compatible with number");
}