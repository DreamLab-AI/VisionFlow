...
mod tests {
    ...
    fn test_percentage_based_boundary_detection_small_viewport() {
        ...
        let nodes = vec![
            Node { id: 1, position: Vec3::new(85.0, 0.0, 0.0) }, // Not at boundary
            Node { id: 2, position: Vec3::new(95.0, 0.0, 0.0) }, // At boundary (90-100)
            Node { id: 3, position: Vec3::new(100.0, 0.0, 0.0) }, // At boundary (exactly 100%)
            Node { id: 4, position: Vec3::new(105.0, 0.0, 0.0) }, // Beyond boundary
        ];
        
        let (boundary_count, _) = detect_boundary_nodes(&nodes, &params);
        assert_eq!(boundary_count, 2, "Should detect 2 nodes at boundary (95 and 100)");
    }
    ...
    fn test_percentage_based_boundary_detection_large_viewport() {
        ...
        let nodes = vec![
            Node { id: 1, position: Vec3::new(850.0, 0.0, 0.0) }, // Not at boundary
            Node { id: 2, position: Vec3::new(950.0, 0.0, 0.0) }, // At boundary (900-1000)
            Node { id: 3, position: Vec3::new(980.0, 0.0, 0.0) }, // At boundary (the problematic case!)
            Node { id: 4, position: Vec3::new(1000.0, 0.0, 0.0) }, // At boundary (exactly 100%)
            Node { id: 5, position: Vec3::new(1050.0, 0.0, 0.0) }, // Beyond boundary
        ];
        
        let (boundary_count, _) = detect_boundary_nodes(&nodes, &params);
        assert_eq!(boundary_count, 3, "Should detect 3 nodes at boundary (950, 980, 1000)");
    }
    ...
    fn test_original_problem_scenario() {
        ...
        let nodes = vec![
            Node { id: 1, position: Vec3::new(980.0, 0.0, 0.0) }, // This was missed by old system!
            Node { id: 2, position: Vec3::new(990.0, 0.0, 0.0) },
            Node { id: 3, position: Vec3::new(1000.0, 0.0, 0.0) },
        ];
        
        let (boundary_count, _) = detect_boundary_nodes(&nodes, &params);
        assert_eq!(boundary_count, 3, "Should detect all 3 nodes at boundary with new percentage system");
    }
    ...
    fn test_boundary_detection_with_different_viewport_sizes() {
        ...
        for (viewport_bounds, expected_min, expected_max) in test_cases {
            ...
            let nodes = vec![
                Node { id: 1, position: Vec3::new(boundary_min, 0.0, 0.0) },
            ];
            
            let (boundary_count, _) = detect_boundary_nodes(&nodes, &params);
            assert_eq!(boundary_count, 1, "Node at {}% boundary should be detected", 90);
        }
    }
    ...
    fn test_bouncing_detection_logic() {
        ...
        assert!(boundary_percentage > params.auto_balance_config.bouncing_node_percentage,
    ...
    fn test_extreme_distance_detection() {
        ...
        let nodes = vec![
            Node { id: 1, position: Vec3::new(500.0, 0.0, 0.0) },   // Normal
            Node { id: 2, position: Vec3::new(950.0, 0.0, 0.0) },   // Boundary
            Node { id: 3, position: Vec3::new(1500.0, 0.0, 0.0) },  // Extreme
        ];
...
mod integration_tests {
    ...
    fn test_full_auto_balance_scenario() {
        ...
        let nodes = vec![
            Node { id: 1, position: Vec3::new(200.0, 0.0, 0.0) },   // Clustered
            Node { id: 2, position: Vec3::new(300.0, 0.0, 0.0) },   // Clustered
            Node { id: 3, position: Vec3::new(980.0, 0.0, 0.0) },   // At boundary (the key test case!)
            Node { id: 4, position: Vec3::new(995.0, 0.0, 0.0) },   // At boundary
            Node { id: 5, position: Vec3::new(1000.0, 0.0, 0.0) },  // At boundary
            Node { id: 6, position: Vec3::new(1200.0, 0.0, 0.0) },  // Beyond boundary
        ];
        
        let (boundary_count, extreme_count) = detect_boundary_nodes(&nodes, &params);
        
        // With the fixed system, nodes at 980, 995, and 1000 should all be detected as boundary
        assert_eq!(boundary_count, 3, "Should detect 3 boundary nodes with percentage-based system");
        assert_eq!(extreme_count, 1, "Should detect 1 extreme node");
    ...
    fn test_parameter_adjustment_with_clamping() {
        ...
        assert!(new_repel_k <= 100.0, "repel_k should be clamped to max 100.0");
