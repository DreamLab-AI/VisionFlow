...
- Sampling that depends on wall-clock time (must be a keyed function of the request id)
- Sampling the requesting agent can compute or evade: any scheme keyed only on values the agent controls (an unkeyed hash of the agent-chosen `d` tag) lets it grind for an id that is never sampled — the relay secret must be an input
