...
const createGraphSettingsSubsections = (graphName: 'logseq' | 'visionflow') => ({
  ...
  [`${graphName}Physics`]: {
    ...
    settings: {
      ...
      maxVelocity: { label: 'Max Velocity', type: 'slider', min: 0.1, max: 10.0, step: 0.1, unit: 'units/s', path: `visualisation.graphs.${graphName}.physics.maxVelocity`, description: 'Speed limit for node movement.' },
      repulsionStrength: { label: 'Repulsion Strength', type: 'slider', min: 1, max: 500, step: 1, path: `visualisation.graphs.${graphName}.physics.repulsionStrength`, description: 'Force pushing unconnected nodes apart.' },
      springStrength: { label: 'Spring Strength', type: 'slider', min: 0.001, max: 0.05, step: 0.001, path: `visualisation.graphs.${graphName}.physics.springStrength`, description: 'Edge stiffness (higher = tighter connections).' },
