...
import type { BotsAgent, BotsCommunication, TokenUsage } from '../types/BotsTypes';

const logger = createLogger('MockBotsDataProvider');

export class MockBotsDataProvider {
  ...
  connect(): Promise<void> {
    return new Promise((resolve) => {
      logger.info('Mock bots data provider connected');

      // Start simulating updates
      this.startSimulation();

      setTimeout(() => {
        this.emit('welcome', { message: 'Connected to mock bots' });
        resolve();
      }, 100);
    });
  }
  ...
  disconnect() {
    ...
    logger.info('Mock bots data provider disconnected');
  }
  ...
  private generateCommunications(): BotsCommunication[] {
    const agents = Array.from(this.agents.values());
  ...
  getCommunications(): Promise<BotsCommunication[]> {
