...
>(({ className, children, size, showCloseButton = true, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {showCloseButton && (
        <DialogPrimitive.Close className="absolute rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 disabled:pointer-events-none" style={{ right: '1rem', top: '1rem' }}>
          <X className="h-4 w-4" />
