...
class ApiService {
  ...
  /**
   * Get mock bots data for development when backend is unavailable
   */
  private getMockBotsData(): any {
    const mockNodes = [
  ...
  private getMockGraphData(): any {
    return {
      nodes: [
        ...
        {
          id: "2",
          metadataId: "Agent Bots Visualization",
          label: "Agent Bots Visualization",
          data: {
          ...
          metadata: {
            metadataId: "Agent Bots Visualization",
            name: "Agent Bots Visualization",
            nodeSize: "15",
            lastModified: new Date().toISOString(),
            fileSize: "800",
            fileName: "bots-viz.md",
            hyperlinkCount: "3"
          },
          size: 15
        }
  ...
  public async getBotsData(): Promise<any> {
    try {
      ...
      logger.warn('Failed to get bots data from backend, using mock data', error);
      return this.getMockBotsData();
    }
  }
